...
function pl({ children: e }) {
  ...
  return jsxs(ul.Provider, {
    value: l,
    children: [e, jsx(up, { notifications: n.notifications, onDismiss: a })],
  });
}
function up({ notifications: e, onDismiss: t }) {
  ...
  return n.length === 0
    ...
    : jsx('div', {
        ...
        children: n.map((r) => jsx(pp, { notification: r, onDismiss: () => t(r.id) }, r.id)),
      });
}
function pp({ notification: e, onDismiss: t }) {
...
var cr = class extends Component {
  constructor(n) {
    super(n);
    d$1(this, 'resetError', () => {
    ...
    if (this.state.hasError) {
      ...
      return n
        ? typeof n == 'function'
          ? n(r, this.resetError)
          : n
        : jsx(fp, { error: r, errorInfo: i, showDetails: o, onReset: this.resetError });
    }
    return this.props.children;
  }
};
function fp({ error: e, errorInfo: t, showDetails: n, onReset: o }) {
  let r = {
      display: 'flex',
      flexDirection: 'column',
      alignItems: 'center',
...
function qb({ message: e, details: t, onRetry: n, className: o = '' }) {
  let r = {
      display: 'flex',
      flexDirection: 'column',
      alignItems: 'center',
...
function Xb({ feature: e, description: t, className: n = '' }) {
  let o = {
      display: 'flex',
      alignItems: 'center',
      gap: '8px',
...
function ml({ document: e, onChange: t, onSelectionChange: n }) {
  let o = useMemo(() => new C(), []),
    [r, i] = useState({
      context: null,
      table: null,
...
var hl = ({ headings: e, onHeadingClick: t, onClose: n, topOffset: o = 0 }) => {
...
function xp(e) {
  ...
  return xl[Math.abs(t) % xl.length];
}
var Sp = 340,
  kp = 8,
  pr = {
  ...
  kl = ({
    ...
    let [S, w] = useState(null),
      ...
      [V, $] = useState(null),
      [L, W] = useState(null),
      [O, K] = useState(new Map()),
      [C, B] = useState(false),
      ee = useRef(new Set()),
      he = useRef(null),
      fe = useRef(new Map()),
      Ie = useMemo(() => e.filter((v) => !(v.parentId != null || (v.done && !p))), [e, p]),
      Te = useMemo(() => {
        let v = new Map();
        for (let z of e)
          if (z.parentId != null) {
            let D = v.get(z.parentId);
            D ? D.push(z) : v.set(z.parentId, [z]);
          }
        return v;
      }, [e]),
      Re = (v) => Te.get(v) ?? [],
      P = useCallback(() => {
        let v = g?.current;
        if (!v) return;
        let z = v.querySelector('.paged-editor__pages');
        if (!z) return;
        let D = v.getBoundingClientRect(),
          ie = v.scrollTop,
          de = [];
        for (let me of Ie) {
          let Se = z.querySelector(`[data-comment-id="${me.id}"]`);
          if (Se) {
            let pe = Se.getBoundingClientRect();
            de.push({
              id: `comment-${me.id}`,
              targetY: pe.top - D.top + ie,
              height: fe.current.get(`comment-${me.id}`)?.offsetHeight || 80,
            });
          }
        }
        (t.forEach((me, Se) => {
          let pe = `tc-${me.revisionId}-${Se}`,
            $e = z.querySelector(`[data-revision-id="${me.revisionId}"]`);
          if ($e) {
            let Qe = $e.getBoundingClientRect();
            de.push({
              id: pe,
              targetY: Qe.top - D.top + ie,
              height: fe.current.get(pe)?.offsetHeight || 80,
            });
          }
        }),
          f &&
            ...
            de.push({
              ...
              targetY: m,
              height: fe.current.get('new-comment-input')?.offsetHeight || 120,
            }),
          de.sort((me, Se) => me.targetY - Se.targetY));
        let ue = new Map(),
          Ee = 0;
        for (let me of de) {
          let Se = Math.max(me.targetY, Ee + kp);
          (ue.set(me.id, Se), (Ee = Se + me.height));
        }
        K(ue);
      }, [Ie, t, g, f, m]);
    (useEffect(() => {
      ...
      let D = (ie) => {
        let de = ie.target;
        if (!he.current?.contains(de)) {
          if (z.contains(de)) {
            let ue = de.closest('[data-comment-id]');
            if (ue?.dataset.commentId) {
              ($(`comment-${ue.dataset.commentId}`), n?.(Number(ue.dataset.commentId)));
              return;
            }
            let Ee = de.closest('.docx-insertion') || de.closest('.docx-deletion');
            if (Ee?.dataset.revisionId) {
              let me = Number(Ee.dataset.revisionId),
                Se = t.findIndex((pe) => pe.revisionId === me);
              if (Se >= 0) {
      ...
      useEffect(() => {
        ...
        return (
          ie.observe(v),
          () => {
            (clearTimeout(z), clearTimeout(D), ie.disconnect());
          }
        );
      }, [P, g]),
      useEffect(() => {
        let v = requestAnimationFrame(P);
        return () => cancelAnimationFrame(v);
      }, [V, f, P]),
      useEffect(() => {
        let v = [];
        if (V) {
          let de = fe.current.get(V);
          de && v.push(de);
        }
        let z = fe.current.get('new-comment-input');
        if ((z && v.push(z), v.length === 0)) return;
        let D,
          ie = new ResizeObserver(() => {
            (cancelAnimationFrame(D), (D = requestAnimationFrame(P)));
          });
        for (let de of v) ie.observe(de);
        return () => {
    ...
    let I = () => {
      ...
      y = (v, z) => {
        ($(V === v ? null : v), W(null), z !== void 0 && n?.(z));
      },
      M = O.size > 0,
      F = (v, z = 32) => ({
        width: z,
      ...
      U = (v, z, D) => {
        let ie = ee.current.has(v);
        return (
          D !== void 0 && ee.current.add(v),
          {
            ...(M
              ? D !== void 0
              ...
              : { marginBottom: 6 }),
            padding: z ? '10px 12px' : '8px 10px',
            borderRadius: 8,
            backgroundColor: z ? '#fff' : '#f8fbff',
            cursor: 'pointer',
            boxShadow: z
                  ...
                  children: [
                    jsx('input', {
                      ...
                      onKeyDown: (D) => {
                        (D.stopPropagation(),
                          D.key === 'Enter' &&
                            (D.preventDefault(), k.trim() && z && z(v, k.trim()), E(''), w(null)),
                    ...
                    jsxs('div', {
                      ...
                      children: [
                        ...
                        jsx('button', {
                          onClick: (D) => {
                            (D.stopPropagation(), k.trim() && z && z(v, k.trim()), E(''), w(null));
                          },
                          disabled: !k.trim(),
                          style: A(!!k.trim()),
                          children: 'Reply',
      ...
      j = (v) => {
        ...
        return jsxs(
          ...
          {
            ref: (ue) => {
              ue ? fe.current.set(D, ue) : fe.current.delete(D);
            },
            'data-comment-id': v.id,
            className: 'docx-comment-card',
            onClick: () => y(D, v.id),
            onMouseDown: (ue) => ue.stopPropagation(),
            style: { ...U(D, ie, de), opacity: M && de === void 0 ? 0 : v.done ? 0.6 : 1 },
            children: [
              jsxs('div', {
                style: { display: 'flex', alignItems: 'flex-start', gap: 10 },
                children: [
                  jsx('div', { style: F(v.author || 'U'), children: Ra(v.author || 'U') }),
                  jsxs('div', {
                    style: { flex: 1, minWidth: 0 },
                    children: [
                      jsx('div', {
                  ...
                  ie &&
                    jsxs('div', {
                      ...
                      children: [
                        ...
                        jsx('button', {
                          onClick: (ue) => {
                            (ue.stopPropagation(), W(L === D ? null : D));
                          },
                          title: 'More options',
                          style: pr,
      ...
      xe = (v, z) => {
        ...
        return jsxs(
          ...
          {
            ref: (me) => {
              me ? fe.current.set(ie, me) : fe.current.delete(ie);
            },
            ...
            children: [
              jsxs('div', {
                style: { display: 'flex', alignItems: 'flex-start', gap: 10 },
                children: [
                  jsx('div', { style: F(D), children: Ra(D) }),
                  jsxs('div', {
                    style: { flex: 1, minWidth: 0 },
                    children: [
                      jsx('div', {
                  ...
                  de &&
                    jsxs('div', {
                      ...
                      children: [
                        jsx('button', {
                          onClick: (me) => {
                            (me.stopPropagation(), l?.(v.from, v.to));
                          },
                          title: 'Accept',
                          style: pr,
                          children: jsx(d, { name: 'check', size: 20 }),
                        ...
                        jsx('button', {
                          onClick: (me) => {
                            (me.stopPropagation(), u?.(v.from, v.to));
                          },
                          title: 'Reject',
                          style: pr,
      ...
      children: jsxs('div', {
        ...
        children: [
          f &&
            jsxs('div', {
              ref: (v) => {
                v ? fe.current.set('new-comment-input', v) : fe.current.delete('new-comment-input');
              },
              style: {
                ...(M
                  ? O.get('new-comment-input') !== void 0
                    ? { position: 'absolute', top: O.get('new-comment-input'), left: 0, right: 0 }
...
var Tp = 15840,
  ...
  Rp = 567,
  vl = 20,
  Pp = 'var(--doc-text-muted)',
  Mp = 'var(--doc-text-subtle)',
  Ep = 'var(--doc-primary)',
  Ip = 'var(--doc-primary)',
  Fp = 'var(--doc-primary-hover)';
function Rl({
  sectionProps: e,
  zoom: t = 1,
  editable: n = false,
  ...
  let [l, u] = useState(null),
    ...
    E = useCallback(
      ($) => {
        ...
        if (l === 'topMargin') {
          let K = f - b - 720,
            C = Math.max(0, Math.min(O, K));
          o?.(Math.round(C));
        } else if (l === 'bottomMargin') {
          let K = f - O,
            C = f - m - 720,
            B = Math.max(0, Math.min(K, C));
          r$1?.(Math.round(B));
        }
      },
      [l, t, f, m, b, o, r$1]
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', {
        ...
        children: _.map(($, L) => jsx(Lp, { tick: $ }, L)),
      }),
      jsx(Tl, {
        type: 'topMargin',
        position: S,
...
var El = je({ name: 'text', schemaNodeName: 'text', nodeSpec: { group: 'inline' } });
function Il(e, t) {
  if (!e) return {};
  let n$1 = {};
  if (e.fontFamily) {
...
function Fl(e, t) {
  ...
  let n = {};
  if (
    (e.alignment && (n.textAlign = Hp(e.alignment)),
  ...
  if (
    ...
    e.indentFirstLine !== void 0 &&
      (e.hangingIndent
        ...
        : (n.textIndent = B$1(r(e.indentFirstLine)))),
    e.borders &&
      (e.borders.top && Object.assign(n, to(e.borders.top, 'Top', t)),
      e.borders.bottom && Object.assign(n, to(e.borders.bottom, 'Bottom', t)),
      e.borders.left && Object.assign(n, to(e.borders.left, 'Left', t)),
      e.borders.right && Object.assign(n, to(e.borders.right, 'Right', t))),
    e.shading)
  ) {
    let o = Ll(e.shading, t);
...
function Wp(e, t, n) {
  ...
  return t
    ...
    : `docx-list-numbered ${$p(n)} docx-list-level-${o}`;
}
var _p = {
  content: 'inline*',
  group: 'block',
  ...
  toDOM(e) {
    let t = e.attrs,
      n = Op(t),
      o = Wp(t.numPr, t.listIsBullet, t.listNumFmt),
      r = {};
    return (
      n && (r.style = n),
      o && (r.class = o),
...
function fr(e, t = 'auto') {
  return (n, o) => Bl({ lineSpacing: e, lineSpacingRule: t })(n, o);
}
function Vp(e = 720) {
  return (t, n) => {
    let { $from: o, $to: r } = t.selection;
  ...
  onSchemaReady(e) {
    ...
    return {
      commands: {
        setAlignment: (n) => Mo(n),
        alignLeft: () => Mo('left'),
        alignCenter: () => Mo('center'),
        alignRight: () => Mo('right'),
        alignJustify: () => Mo('both'),
        setLineSpacing: (n, o) => fr(n, o),
        singleSpacing: () => fr(240),
        oneAndHalfSpacing: () => fr(360),
        doubleSpacing: () => fr(480),
        setSpaceBefore: (n) => jt('spaceBefore', n),
        setSpaceAfter: (n) => jt('spaceAfter', n),
        increaseIndent: (n) => Vp(n),
        decreaseIndent: (n) => jp(n),
        setIndentLeft: (n) => jt('indentLeft', n > 0 ? n : null),
        setIndentRight: (n) => jt('indentRight', n > 0 ? n : null),
        setIndentFirstLine: (n, o) =>
          Bl({ indentFirstLine: n > 0 ? n : null, hangingIndent: o ?? false }),
    ...
    if (b.type.name === 'tableCell' || b.type.name === 'tableHeader') {
      ...
      if (g && g.type.name === 'tableRow') {
        let S = 0,
          w = false;
        g.forEach((k, E, R) => {
          w || (R === n.index(m - 1) ? ((s = S), (w = true)) : (S += k.attrs.colspan || 1));
        });
      }
...
var mf = je({ name: 'table', schemaNodeName: 'table', nodeSpec: cf }),
  gf = je({ name: 'tableRow', schemaNodeName: 'tableRow', nodeSpec: df }),
  hf = je({ name: 'tableCell', schemaNodeName: 'tableCell', nodeSpec: pf }),
  bf = je({ name: 'tableHeader', schemaNodeName: 'tableHeader', nodeSpec: ff }),
  yf = Mt({
    name: 'tablePlugin',
    onSchemaReady(e) {
      let { schema: t } = e;
      ...
      function r(P, I, y = '000000', M = 9360) {
        let F = [],
          ...
          j = { top: N, bottom: N, left: N, right: N };
        for (let xe = 0; xe < P; xe++) {
          let v = [];
          for (let z = 0; z < I; z++) {
            let D = t.nodes.paragraph.create(),
              ie = { colspan: 1, rowspan: 1, borders: j, width: A, widthType: 'dxa' };
            v.push(t.nodes.tableCell.create(ie, D));
          }
          F.push(t.nodes.tableRow.create({ height: U, heightRule: Q }, v));
        }
        ...
        return (y, M) => {
          ...
          if (M) {
            ...
            let j = r(P, I, A, N),
              ...
              v = y.doc.resolve(Q).nodeBefore?.type.name === 'table',
              z = v ? [G, j, G] : [j, G],
              D = y.tr.insert(Q, z),
              ie = Q + 1;
            v && (ie += G.nodeSize);
            let ue = ie + 1 + 1;
            (D.setSelection(TextSelection.create(D.doc, ue)), M(D.scrollIntoView()));
          }
          return true;
        };
      }
      ...
      function l(P, I) {
        ...
        if (I) {
          let M = P.tr,
            F = y.tablePos + 1;
          for (let U = 0; U < y.rowIndex; U++) F += y.table.child(U).nodeSize;
          ...
          y.table.forEach((j) => {
            if (j.type.name === 'tableRow') {
              ...
              if (
                (j.forEach((v) => {
                  if (xe === y.columnIndex) {
                    let z = t.nodes.paragraph.create(),
                      D = o(v, { colspan: 1, rowspan: 1 });
                    Q === 0 && ((D.width = A), (D.widthType = 'pct'));
                    let ie = t.nodes.tableCell.create(D, z);
                    M = M.insert(G, ie);
                  }
                  ((G += v.nodeSize), (xe += v.attrs.colspan || 1));
                }),
                xe <= y.columnIndex)
              ) {
                let v = t.nodes.paragraph.create(),
                  z = o(j.child(j.childCount - 1) ?? null, { colspan: 1, rowspan: 1 });
                Q === 0 && ((z.width = A), (z.widthType = 'pct'));
                let D = t.nodes.tableCell.create(z, v);
                M = M.insert(G, D);
              }
              Q++;
            }
            ...
            if (j && j.type.name === 'tableRow') {
              ...
              j.forEach((D) => {
                ((D.type.name === 'tableCell' || D.type.name === 'tableHeader') &&
                  (M = M.setNodeMarkup(z, void 0, { ...D.attrs, width: A, widthType: 'pct' })),
                  (z += D.nodeSize));
              });
            }
            let G = j?.childCount ?? F,
              xe = N.attrs.width || 9360,
              v = Math.floor(xe / Math.max(1, G));
            M = M.setNodeMarkup(y.tablePos, void 0, { ...N.attrs, columnWidths: Array(G).fill(v) });
          }
          I(M.scrollIntoView());
        }
        return true;
        ...
        if (I) {
          ...
          y.table.forEach((j) => {
            if (j.type.name === 'tableRow') {
              ...
              if (
                (j.forEach((z) => {
                  if (((G += z.nodeSize), (xe += z.attrs.colspan || 1), !v && xe > y.columnIndex)) {
                    let D = t.nodes.paragraph.create(),
                      ie = o(z, { colspan: 1, rowspan: 1 });
                    Q === 0 && ((ie.width = A), (ie.widthType = 'pct'));
                    let de = t.nodes.tableCell.create(ie, D);
                    ((M = M.insert(G, de)), (v = true));
                  }
                }),
                !v)
              ) {
          ...
          (y.table.forEach((j) => {
            if (j.type.name === 'tableRow') {
              ...
              j.forEach((v) => {
                let z = G,
                  D = G + v.nodeSize,
                  ie = v.attrs.colspan || 1;
                (xe <= y.columnIndex && y.columnIndex < xe + ie && U.push({ start: z, end: D }),
                  (G = D),
                  (xe += ie));
              });
            }
            ...
            U.reverse().forEach(({ start: j, end: G }) => {
              M = M.delete(j, G);
            }));
          let N = M.doc.nodeAt(y.tablePos);
          if (N && N.type.name === 'table') {
      ...
      function f(P, I) {
        ...
        if (I) {
          let M = y.tablePos + 1,
            ...
            U = CellSelection.create(P.doc, F.pos, A.pos);
      ...
      function m(P, I) {
        ...
        if (I) {
          let F = y.tablePos + 1;
          for (let j = 0; j < y.rowIndex; j++) {
            let G = y.table.child(j);
            F += G.nodeSize;
          }
          let A = y.table.child(y.rowIndex),
            U = F + 1,
            Q = F + A.nodeSize - 2,
            N = CellSelection.create(P.doc, U, Q);
      ...
      function b(P, I) {
        ...
        if (I) {
          let M = y.tablePos + 1,
            ...
            U = M + 1;
          for (let G = 0; G < y.columnIndex && G < F.childCount; G++) U += F.child(G).nodeSize;
          let Q = M;
          ...
          let j = CellSelection.create(P.doc, U, N);
          I(P.tr.setSelection(j));
        }
        return true;
      }
      function g(P) {
      ...
      function S(P, I) {
        ...
        return (
          P.forEach((U, Q, N) => {
            if (U.type.name !== 'tableRow') return;
            let j = 0;
            (U.forEach((G, xe) => {
              let v = I + Q + xe + 2,
                z = G.attrs.colspan || 1;
              (y.set(v, { rowIdx: N, colIdx: j, colspan: z, pos: v, node: G }),
                M.set(`${N},${j}`, v),
                (j += z));
            }),
              (A = Math.max(A, j)));
        ...
        return (y, M) => {
          ...
          if (M) {
            ...
            for (let { pos: Se } of v) {
              let pe = G.get(Se);
              pe &&
                ((z = Math.min(z, pe.rowIdx)),
                (D = Math.max(D, pe.rowIdx)),
                (ie = Math.min(ie, pe.colIdx)),
                (de = Math.max(de, pe.colIdx + pe.colspan - 1)));
            }
            let ue = new Map(),
              Ee = (Se, pe) => ue.get(Se) ?? { ...pe.attrs },
              me = (Se, pe) => {
                ue.set(Se, pe);
              };
            for (let { pos: Se } of v) {
              ...
              switch (P) {
                case 'all':
                  ...
                  break;
                case 'outside':
                  We = { top: $e ? N : j, bottom: Qe ? N : j, left: Ge ? N : j, right: tt ? N : j };
                  break;
                case 'inside':
                  We = { top: $e ? j : N, bottom: Qe ? j : N, left: Ge ? j : N, right: tt ? j : N };
                  break;
                case 'none':
                  We = { top: j, bottom: j, left: j, right: j };
                  break;
              }
              let Je = Ee(Se, pe.node),
                it = Je.borders || {};
              if ((me(Se, { ...Je, borders: { ...it, ...We } }), We.top)) {
                let ze = xe.get(`${pe.rowIdx - 1},${pe.colIdx}`);
                if (ze !== void 0) {
                  let ct = G.get(ze),
                    De = Ee(ze, ct.node),
                    Rt = De.borders || {};
              ...
              if (We.left) {
                let ze = xe.get(`${pe.rowIdx},${pe.colIdx - 1}`);
                if (ze !== void 0) {
                  let ct = G.get(ze),
                    De = Ee(ze, ct.node),
                    Rt = De.borders || {};
              ...
              if (We.right) {
                let ze = xe.get(`${pe.rowIdx},${pe.colIdx + pe.colspan}`);
                if (ze !== void 0) {
                  let ct = G.get(ze),
                    De = Ee(ze, ct.node),
                    Rt = De.borders || {};
            ...
            for (let [Se, pe] of ue) A.setNodeMarkup(A.mapping.map(Se), void 0, pe);
            M(A.scrollIntoView());
          }
          return true;
        };
        ...
        return (I, y) => {
          ...
          if (y) {
            ...
            for (let { pos: Q, node: N } of A)
              F.setNodeMarkup(F.mapping.map(Q), void 0, { ...N.attrs, backgroundColor: U });
            y(F.scrollIntoView());
          }
          return true;
        };
        ...
        return (M, F) => {
          ...
          if (F) {
            let U = M.tr,
              ...
              { cellByPos: xe, cellByRC: v } = S(A.table, A.tablePos),
              z = new Map(),
              D = (ue, Ee) => z.get(ue) ?? { ...Ee.attrs },
              ie = (ue, Ee) => z.set(ue, Ee),
              de = {
            ...
            for (let { pos: ue, node: Ee } of Q) {
              let me = xe.get(ue),
                Se = D(ue, Ee),
                pe = Se.borders || {},
                $e = P === 'all' ? G : [P],
                Qe = y ? { top: j, bottom: j, left: j, right: j } : { ...pe };
            ...
            for (let [ue, Ee] of z) U.setNodeMarkup(U.mapping.map(ue), void 0, Ee);
            F(U.scrollIntoView());
          }
          return true;
        };
      ...
      function R(P) {
        return (I, y) => {
          ...
          if (y) {
            let F = I.tr,
              A = g(I);
            for (let { pos: U, node: Q } of A)
              F.setNodeMarkup(F.mapping.map(U), void 0, { ...Q.attrs, verticalAlign: P });
            y(F.scrollIntoView());
          }
          return true;
        };
        ...
        return (I, y) => {
          ...
          if (y) {
            let F = I.tr,
              A = g(I);
            for (let { pos: U, node: Q } of A) {
              let j = { ...(Q.attrs.margins || {}), ...P };
              F.setNodeMarkup(F.mapping.map(U), void 0, { ...Q.attrs, margins: j });
            }
            y(F.scrollIntoView());
          }
          return true;
        ...
        return (I, y) => {
          ...
          if (y) {
            let F = I.tr,
              A = g(I);
            for (let { pos: U, node: Q } of A)
              F.setNodeMarkup(F.mapping.map(U), void 0, { ...Q.attrs, textDirection: P });
            y(F.scrollIntoView());
          }
          return true;
        };
      ...
      function $() {
        return (P, I) => {
          ...
          if (I) {
            let M = P.tr,
              F = g(P);
            for (let { pos: A, node: U } of F)
              M.setNodeMarkup(M.mapping.map(A), void 0, { ...U.attrs, noWrap: !U.attrs.noWrap });
            I(M.scrollIntoView());
          }
          return true;
        };
      ...
      function L(P, I) {
        return (y, M) => {
          ...
          if (M) {
            let A = y.tr,
              { $from: U } = y.selection;
            for (let Q = U.depth; Q > 0; Q--) {
              let N = U.node(Q);
              if (N.type.name === 'tableRow') {
                let j = U.before(Q),
                  G = { ...N.attrs, height: P, heightRule: P ? I || 'atLeast' : null };
                return (A.setNodeMarkup(j, void 0, G), M(A.scrollIntoView()), true);
              }
            }
          }
          return true;
        ...
        return (P, I) => {
          ...
          if (I) {
            let M = P.tr,
              F = y.table,
              A = y.columnCount,
              U = F.attrs.columnWidths,
              Q = U ? U.reduce((xe, v) => xe + v, 0) : 9360,
              N = Math.floor(Q / A),
              j = y.tablePos + 1;
            F.forEach((xe) => {
              if (xe.type.name === 'tableRow') {
                let v = j + 1;
                xe.forEach((z) => {
                  ((z.type.name === 'tableCell' || z.type.name === 'tableHeader') &&
                    (M = M.setNodeMarkup(v, void 0, {
                      ...z.attrs,
                      width: N,
                      widthType: 'dxa',
                      colwidth: null,
        ...
        return (I, y) => {
          ...
          if (y) {
            let F = I.tr,
              ...
              j = P.conditionals ?? {},
              G = P.tableBorders;
            F = F.setNodeMarkup(U, void 0, { ...A.attrs, styleId: P.styleId });
            let xe = 0,
              v = U + 1;
            for (let z = 0; z < Q; z++) {
              let D = A.child(z),
              ...
              ie
                ...
                : de
                  ? (me = 'lastRow')
                  : (ue && (me = xe % 2 === 0 ? 'band1Horz' : 'band2Horz'), xe++);
              let Se = v + 1;
              for (let pe = 0; pe < Ee; pe++) {
                ...
                for (let De of ct)
                  Je?.borders && Je.borders[De] !== void 0
                    ? (ze[De] = Je.borders[De])
                    : G &&
                      ((De === 'top' && z > 0) || (De === 'bottom' && z < Q - 1)
                        ? (ze[De] = G.insideH ?? G[De])
                        : (De === 'left' && pe > 0) || (De === 'right' && pe < Ee - 1)
        ...
        return (P, I) => {
          ...
          if (I) {
            ...
            for (let A = F.depth; A > 0; A--) {
              ...
              if (U.type.name === 'tableRow') {
                let Q = F.before(A),
                  N = { ...U.attrs, isHeader: !U.attrs.isHeader };
        ...
        return (I, y) => {
          ...
          if (y) {
            let F = I.tr,
              ...
              { cellByPos: N, cellByRC: j } = S(M.table, M.tablePos),
              G = new Map(),
            ...
            for (let { pos: D, node: ie } of A) {
              let de = N.get(D),
                ue = xe(D, ie),
                Ee = ue.borders || {},
                me = {};
              for (let Se of ['top', 'bottom', 'left', 'right']) {
                let pe = { ...Q, ...Ee[Se], color: { rgb: U } };
                if (((me[Se] = pe), de)) {
                  let $e = z[Se],
                    Qe = Se === 'right' ? de.colIdx + de.colspan : de.colIdx + $e.dCol,
                    Ge = j.get(`${de.rowIdx + $e.dRow},${Qe}`);
                  if (Ge !== void 0) {
                    let tt = N.get(Ge),
                      We = xe(Ge, tt.node),
                      Je = We.borders || {};
                    v(Ge, { ...We, borders: { ...Je, [$e.adjSide]: pe } });
                  }
                }
              }
              v(D, { ...ue, borders: { ...Ee, ...me } });
            }
      ...
      function he(P) {
        return (I, y) => {
          ...
          if (y) {
            let F = I.tr,
              A = g(I),
              U = { style: 'single', color: { rgb: '000000' } },
              { cellByPos: Q, cellByRC: N } = S(M.table, M.tablePos),
              j = new Map(),
              G = (z, D) => j.get(z) ?? { ...D.attrs },
              xe = (z, D) => j.set(z, D),
              v = {
                top: { adjSide: 'bottom', dRow: -1, dCol: 0 },
                bottom: { adjSide: 'top', dRow: 1, dCol: 0 },
            ...
            for (let { pos: z, node: D } of A) {
              let ie = Q.get(z),
                de = G(z, D),
                ue = de.borders || {},
                Ee = {};
              for (let me of ['top', 'bottom', 'left', 'right']) {
                let Se = { ...U, ...ue[me], size: P };
                if (((Ee[me] = Se), ie)) {
                  let pe = v[me],
                    $e = me === 'right' ? ie.colIdx + ie.colspan : ie.colIdx + pe.dCol,
                    Qe = N.get(`${ie.rowIdx + pe.dRow},${$e}`);
                  if (Qe !== void 0) {
                    let Ge = Q.get(Qe),
                      tt = G(Qe, Ge.node),
                      We = tt.borders || {};
                    xe(Qe, { ...tt, borders: { ...We, [pe.adjSide]: Se } });
                  }
                }
              }
              xe(z, { ...de, borders: { ...ue, ...Ee } });
            }
            for (let [z, D] of j) F.setNodeMarkup(F.mapping.map(z), void 0, D);
            y(F.scrollIntoView());
          }
          return true;
        };
      ...
      function Ie() {
        return (P) => {
          ...
          let A = I.node(F - 1),
            U = I.index(F - 1),
            Q = U > 0 ? A.child(U - 1) : null,
            N = U + 1 < A.childCount ? A.child(U + 1) : null,
            j = Q?.type.name === 'table',
            G = N?.type.name === 'table';
          return !!(j || G);
        };
      }
      let Te = new PluginKey('activeCell'),
        Re = new Plugin({
          key: Te,
          props: {
            decorations(P) {
      ...
      return {
        ...
        keyboardShortcuts: { Backspace: n(fe(), Ie()), Delete: n(fe(), Ie()) },
        commands: {
          ...
          setCellVerticalAlign: (P) => R(P),
          setCellMargins: (P) => _(P),
          setCellTextDirection: (P) => V(P),
          toggleNoWrap: () => $(),
          setRowHeight: (P, I) => L(P, I),
          toggleHeaderRow: () => B(),
          distributeColumns: () => W(),
          autoFitContents: () => O(),
...
var Ec = Mt({
  ...
  onSchemaReady() {
    return {
      commands: {
        toggleBulletList: () => xf,
        toggleNumberedList: () => Sf,
        increaseListLevel: () => kf,
        decreaseListLevel: () => wf,
        removeList: () => Cf,
      },
      keyboardShortcuts: {
        Tab: La(vc(), Pf(), Ef()),
...
var Hf = (e, t) => {
  ...
  Lc = new Set(['fontFamily', 'fontSize', 'textColor']),
  zf = (e, t, n) => {
    let { $from: o } = e.selection,
      r = o.parent.type.name === 'paragraph' ? o.parent : null,
      a = (e.storedMarks || o.marks()).filter((u) => Lc.has(u.type.name)),
      s = null;
    if (
      !splitBlock(
        e,
      ...
      if (c.type.name === 'paragraph') {
        ...
        if (
          ...
          if (m.length === 0 && r) {
            ...
            b && (m = Ea(b, e.schema).filter((S) => Lc.has(S.type.name)));
          }
          if (m.length > 0) {
    ...
    onSchemaReady(e) {
      return {
        keyboardShortcuts: {
          ...baseKeymap,
          Enter: zf,
          Backspace: Fc(deleteSelection, Hf, joinBackward),
          Delete: Fc(deleteSelection, joinForward),
          'Mod-a': selectAll,
...
var gr = new PluginKey('selectionTracker');
function ao(e) {
  ...
  return new Plugin({
    ...
    state: {
      ...
      apply(t, n, o, r) {
        ...
        let i = ao(r);
        return (e && !Vf(n, i) && setTimeout(() => e(i), 0), i);
      },
    },
    view() {
      return {
...
function Vf(e, t) {
  return (
    ...
    _f(e.activeCommentIds, t.activeCommentIds) &&
...
var Ac = Mt({
  ...
  onSchemaReady(e, t) {
    return {
      plugins: [hr(t.onSelectionChange)],
      commands: { extractSelectionContext: () => (n, o) => (ao(n), true) },
    };
  },
});
var Gf = new PluginKey('imageDrag'),
  Hc = Mt({
    name: 'imageDrag',
    onSchemaReady(e) {
      return {
...
function so(e = {}) {
  ...
  return (
    ...
    o('text', El()),
    o('paragraph', Al()),
    o('history', Hl({ depth: e.historyDepth, newGroupDelay: e.historyNewGroupDelay })),
    o('bold', zl()),
    o('italic', $l()),
    o('underline', Vl()),
    o('strike', jl()),
    ...
    o('sdt', yc()),
    o('math', xc()),
    t.has('table') || n.push(...Pc()),
    o('list', Ec()),
    o('baseKeymap', Bc()),
    o('selectionTracker', Ac({ onSelectionChange: e.onSelectionChange })),
    n
  );
}
var Sn = class {
  ...
  for (let i of n)
    if (i.type === 'paragraph') {
      let a = zc(i),
        s = kr(i, r);
      o.push(s);
      for (let l of a) o.push(Oc(l, r));
      sm(i) && o.push(ye.node('pageBreak'));
...
function kr(e, t, n, o) {
  ...
  for (let d of e.content) {
    if (d.type === 'commentRangeStart') s.add(d.id);
      ...
      let c = yr(d, 'insertion', u, t);
      i.push(...c);
    } else if (d.type === 'deletion') {
      let c = yr(d, 'deletion', u, t);
      i.push(...c);
...
function yr(e, t, n, o) {
  ...
  for (let a of e.content)
    a.type === 'run' ? r.push(...Aa(a, n, o)) : a.type === 'hyperlink' && r.push(...Da(a, n, o));
...
function Zf(e, t) {
  ...
  if (t) {
    ...
    ((r.alignment = n?.alignment),
      ...
      (r.defaultTextFormatting = xr(n?.runProperties, t)));
...
function Ht(e, t, n) {
  ...
  if (!r) return;
  let i = xr(r.pPr?.runProperties, e),
    a = xr(r.rPr, e),
    s = on(i, a);
...
function Ba(e, t) {
  let n = Jf(e),
    ...
    w = e.rows.map((k, E) => {
      let R = E === 0 && !!a?.firstRow,
        _ = E === g - 1 && !!a?.lastRow,
        V = f && !R && !_ ? (b % 2 === 0 ? p.band1Horz : p.band2Horz) : void 0;
      return (f && !R && !_ && b++, Qf(k, t, R, o, r, p, V, m, a, l, E, g, S, n, d));
    });
  return ye.node('table', c, w);
}
function Qf(e, t, n, o, r, i, a, s, l, u, d, c, p, f, m) {
  let b = {
  ...
  for (let L = 0; L < e.cells.length; L++) {
    ...
    if (o && r && r > 0) {
      let U = 0;
      for (let Q = 0; Q < O && V + Q < o.length; Q++) U += o[V + Q];
      he = Math.round((U / r) * 100);
    }
    if (((V += O), B)) continue;
    let fe = V - O === 0,
      Ie = V === _,
      Te = W.formatting?.conditionalFormat,
      Re = Te?.firstRow ?? E,
      P = Te?.lastRow ?? R,
      I = Te?.firstColumn ?? fe,
      y = Te?.lastColumn ?? Ie,
    ...
    Te?.oddVBand ? (M = i?.band1Vert) : Te?.evenVBand && (M = i?.band2Vert);
    let F = a;
    (k?.oddHBand ? (F = i?.band1Horz) : k?.evenHBand && (F = i?.band2Horz),
      Te?.oddHBand ? (F = i?.band1Horz) : Te?.evenHBand && (F = i?.band2Horz));
    let A = i?.wholeTable;
    ((A = nn(A, F)),
      (A = nn(A, M)),
      Re && (l?.firstRow || k?.firstRow || Te?.firstRow) && (A = nn(A, i?.firstRow)),
      P && (l?.lastRow || k?.lastRow || Te?.lastRow) && (A = nn(A, i?.lastRow)),
      I && (l?.firstColumn || k?.firstColumn || Te?.firstColumn) && (A = nn(A, i?.firstCol)),
      y && (l?.lastColumn || k?.lastColumn || Te?.lastColumn) && (A = nn(A, i?.lastCol)),
      Re &&
        I &&
        (l?.firstRow || k?.firstRow || Te?.firstRow) &&
        (l?.firstColumn || k?.firstColumn || Te?.firstColumn) &&
        (A = nn(A, i?.nwCell)),
      Re &&
        y &&
        (l?.firstRow || k?.firstRow || Te?.firstRow) &&
        (l?.lastColumn || k?.lastColumn || Te?.lastColumn) &&
        (A = nn(A, i?.neCell)),
      P &&
        I &&
        (l?.lastRow || k?.lastRow || Te?.lastRow) &&
        (l?.firstColumn || k?.firstColumn || Te?.firstColumn) &&
        (A = nn(A, i?.swCell)),
      P &&
        y &&
        ...
        (l?.lastColumn || k?.lastColumn || Te?.lastColumn) &&
        (A = nn(A, i?.seCell)),
      $.push(em(W, t, n, he, A, u, S, w, fe, Ie, ee, m)));
  }
  return ye.node('tableRow', b, $);
}
function em(e, t, n, o, r, i, a, s, l, u, d, c) {
  let p = e.formatting,
    f = d ?? 1,
    m = p?.width?.value,
    b = p?.width?.type;
  ...
  for (let $ of e.content)
    $.type === 'paragraph'
      ? _.push(kr($, t, void 0, r?.rPr))
      : $.type === 'table' && _.push(Ba($, t));
  _.length === 0 && _.push(ye.node('paragraph', {}, []));
  let V = n ? 'tableHeader' : 'tableCell';
  return ye.node(V, R, _);
}
function tm(e, t) {
  let n = '',
    o,
...
function rm(e, t) {
  switch (e.type) {
    ...
    case 'tab':
      return [ye.node('tab')];
    case 'drawing':
      return e.image ? [im(e.image)] : [];
    case 'shape': {
      let r = e.shape;
      return r.textBody && r.textBody.content.length > 0 ? [] : [am(r)];
    }
    case 'footnoteRef':
...
function Oc(e, t) {
  ...
  let l = e.margins?.top != null ? c$2(e.margins.top) : 4,
    ...
    p = [];
  for (let f of e.content) p.push(kr(f, t));
  return (
...
function Na(e, t) {
  ...
  for (let r of e)
    if (r.type === 'paragraph') {
      let i = zc(r);
      n.push(kr(r, o));
      for (let a of i) n.push(Oc(a, o));
    } else r.type === 'table' && n.push(Ba(r, o));
    ...
    e.forEach((o) => {
      o.type.name === 'paragraph'
        ...
        : o.type.name === 'table'
          ...
          : o.type.name === 'textBox'
            ? t.push(Mm(o))
            : o.type.name === 'pageBreak' && t.push(lm());
    }),
    t
  );
}
  ...
  return (
    e.forEach((s) => {
      ...
      if (u || d) {
        (r && (n.push(r), (r = null), (i = null)), a && (n.push(a), (a = null)));
        let p = u || d,
          f = s.marks.filter((R) => R.type.name !== 'insertion' && R.type.name !== 'deletion'),
          m = Oa(f),
          b = {
            type: 'run',
      ...
      if (c) {
        ...
        ((a && f === p) || (r && (n.push(r), (r = null), (i = null)), a && n.push(a), (a = fm(c))),
          $c(a, s));
        return;
      }
      if ((a && (n.push(a), (a = null)), s.isText)) {
        ...
        r && i === p
          ? mm(r, s.text || '')
          : (r && n.push(r), (r = jc(s.text || '', s.marks)), (i = p));
      } else
        s.type.name === 'hardBreak'
          ? (r && (n.push(r), (r = null), (i = null)), n.push(gm()))
          : s.type.name === 'image'
            ? (r && (n.push(r), (r = null), (i = null)), n.push(Sm(s)))
            : s.type.name === 'shape'
              ? (r && (n.push(r), (r = null), (i = null)), n.push(km(s)))
              : s.type.name === 'tab'
                ? (r && (n.push(r), (r = null), (i = null)), n.push(hm()))
                : s.type.name === 'field'
                  ? (r && (n.push(r), (r = null), (i = null)), n.push(bm(s, s.marks)))
                  : s.type.name === 'sdt'
                    ? (r && (n.push(r), (r = null), (i = null)), n.push(xm(s)))
                    : s.type.name === 'math' &&
                      (r && (n.push(r), (r = null), (i = null)), n.push(ym(s)));
    }),
    r && n.push(r),
    a && n.push(a),
    n
...
function xm(e) {
  let t = e.attrs,
    ...
    r = _c(e).filter((i) => i.type === 'run' || i.type === 'hyperlink');
...
function Uc(e, t) {
  let n = e.attrs,
    o = [];
  e.forEach((i) => {
    i.type.name === 'tableRow' && o.push(Tm(i, t));
  });
  let r = Cm(n) || void 0;
  if (!r?.borders) {
    let i = wm(o);
...
function Tm(e, t) {
  ...
  return (
    e.forEach((r) => {
      (r.type.name === 'tableCell' || r.type.name === 'tableHeader') && o.push(Rm(r, t));
    }),
    { type: 'tableRow', formatting: vm(n), cells: o }
  );
}
function vm(e) {
  if (e._originalFormatting) {
...
function Rm(e, t) {
  ...
  return (
    e.forEach((r) => {
      r.type.name === 'paragraph' ? o.push(za(r, t)) : r.type.name === 'table' && o.push(Uc(r, t));
    }),
    { type: 'tableCell', formatting: Pm(n), content: o }
  );
}
function Pm(e) {
  if (e._originalFormatting) {
...
function Mm(e) {
  let t = e.attrs,
    n = [];
  e.forEach((a) => {
    a.type.name === 'paragraph' && n.push(za(a));
  });
  let o = {
    type: 'shape',
    shapeType: 'rect',
...
var kt = _n.getCommands(),
  Tr = kt.toggleBold(),
  vr = kt.toggleItalic(),
  Rr = kt.toggleUnderline(),
  Pr = kt.toggleStrike(),
  Mr = kt.toggleSuperscript(),
  Er = kt.toggleSubscript();
function Bo(e) {
  return kt.setTextColor(e);
}
var Ir = kt.clearTextColor();
function Fr(e) {
...
var _r = et.toggleBulletList(),
  Vr = et.toggleNumberedList(),
  jr = et.increaseListLevel(),
  Ur = et.decreaseListLevel();
et.removeList();
function Ho(e, t) {
...
function Wa(e, t) {
  return sd(e, t, 'accept');
}
function _a(e, t) {
  return sd(e, t, 'reject');
}
var cd = {
    display: 'flex',
    alignItems: 'center',
  ...
  dd = { fontWeight: 500, letterSpacing: 0.3 },
  Gm = {
    background: 'none',
    border: 'none',
    color: '#4285f4',
  ...
  pd = forwardRef(function (
    ...
    (useLayoutEffect(() => {
      let $ = () => {
        ...
        k({
          top: O.top - W.top + i.scrollTop,
          left: O.left - W.left + i.scrollLeft,
          width: O.width,
        });
      };
      $();
      ...
      useEffect(() => {
        ...
        let $ = Na(t.content, { styles: o || void 0 }),
          L = new Sn(so());
        (L.buildSchema(), L.initializeRuntime());
        let W = L.getPlugins(),
          O = EditorState.create({ doc: $, schema: ye, plugins: W }),
          K = new EditorView(c.current, {
            state: O,
            dispatchTransaction(C) {
              let B = K.state.apply(C);
              if ((K.updateState(B), C.docChanged && m(true), C.selectionSet || C.docChanged)) {
                let ee = lo(B);
                l?.(ee);
              }
            },
    ...
    (useEffect(() => {
      function $(L) {
        L.key === 'Escape' && (L.preventDefault(), L.stopPropagation(), R());
      }
      return (
...
function Zm(e, t = {}) {
  let {
    ...
    S = useCallback(
      (L, W) => {
        ...
        (O - b.current < o && c.length > 0
          ...
          : p((C) => {
              let B = { state: u, timestamp: O, description: W },
                ee = [...C, B];
    ...
    $ = useCallback((L) => {
      (d((W) => L(W)),
        p((W) => W.map((O) => ({ ...O, state: L(O.state) }))),
        m((W) => W.map((O) => ({ ...O, state: L(O.state) }))));
    }, []);
  return (
    useEffect(() => {
      if (!r) return;
      let L = (O) => {
          ...
          if (
            ((O.ctrlKey || O.metaKey) && O.key === 'y') ||
            ((O.ctrlKey || O.metaKey) && O.key === 'z' && O.shiftKey)
          ) {
            (O.preventDefault(), k());
            return;
          }
...
function yd(e, t = {}) {
  ...
  return Zm(e, { ...t, isEqual: n });
}
var Cn = new PluginKey('suggestionMode'),
  Ti = 'suggestionModeApplied',
  eg = Date.now();
function vi(e) {
  return { revisionId: eg++, author: e.author, date: new Date().toISOString() };
}
function qa(e, t, n, o) {
  try {
    let r = e.resolve(t);
...
function kd(e, t, n, o, r, i, a) {
  ...
  if (
    ...
    return;
  let l = qa(t, n, 'deletion', a.author) || vi(a);
...
function xd(e, t, n, o, r) {
  ...
  let a = e.state.tr;
  a.setMeta(Ti, true);
  let s = qa(e.state.doc, t, 'insertion', r.author) || vi(r);
  if (t !== n) {
    let u = e.state.schema.marks.deletion;
    u && kd(a, e.state.doc, t, n, i, u, r);
  }
...
function Sd(e, t, n) {
  ...
  if (!t) return true;
  let u = e.tr;
  if ((u.setMeta(Ti, true), !a)) {
    kd(u, e.doc, r.pos, i.pos, s, l, o);
  ...
  if (m.marks.some((S) => S.type === l)) {
    ...
    let S = qa(e.doc, c, 'deletion', o.author) || vi(o);
    ...
    props: {
      ...
      handleKeyDown(n, o) {
        return Cn.getState(n.state)?.active
          ? o.key === 'Backspace'
            ? Sd(n.state, n.dispatch, 'backward')
            : o.key === 'Delete'
              ? Sd(n.state, n.dispatch, 'forward')
              : false
          : false;
      },
      handleTextInput(n, o, r, i) {
        let a = Cn.getState(n.state);
        return a?.active ? xd(n, o, r, i, a) : false;
      },
    },
    appendTransaction(n, o, r) {
      let i = Cn.getState(r);
      if (!i?.active) return null;
      let a = n.find((c) => c.docChanged && !c.getMeta(Ti));
      if (!a) return null;
            ...
            g > b &&
              r.doc.nodesBetween(b, g, (S, w) => {
                ...
                if (!S.marks.some((E) => E.type === s || (d && E.type === d))) {
                  let E = Math.max(w, b),
                    R = Math.min(w + S.nodeSize, g);
                  u.addMark(E, R, s.create(l));
                }
              });
          });
        }),
...
function Pd(e, t, n, o = []) {
  let r = n?.getSchema() ?? ye,
    i = e ? Sr(e, { styles: t ?? void 0 }) : wr(),
    a = [...o, ...(n?.getPlugins() ?? [])];
  return EditorState.create({ doc: i, schema: r, plugins: a });
}
function dg(e, t) {
  return t ? Cr(e.doc, t) : null;
}
var ug = forwardRef(function (t, n) {
    let {
        document: o,
        styles: r,
        theme: i,
    ...
    let W = useCallback(() => {
        ...
        let C = {
          ...
          dispatchTransaction: (B) => {
            ...
            let ee = g.current.state.apply(B);
            (g.current.updateState(ee),
              R.current?.(B, ee),
              (B.selectionSet || B.docChanged) && _.current?.(ee));
          },
          handleKeyDown: (B, ee) => L.current?.(B, ee) ?? false,
    ...
    return (
      ...
      useEffect(() => {
        ...
        let B = Pd(o, r, c, d);
        (g.current.updateState(B), _.current?.(B));
      }, [o, r, c, d]),
      useEffect(() => {
        g.current;
      }, [s]),
      useImperativeHandle(
        ...
        () => ({
          ...
          getDocument() {
            return g.current ? dg(g.current.state, S.current) : null;
          },
          focus() {
            g.current?.focus();
          },
          ...
          setSelection(K, C) {
            if (!g.current) return;
            let { state: B, dispatch: ee } = g.current,
              he = B.doc.resolve(K),
              fe = C !== void 0 ? B.doc.resolve(C) : he,
              Ie = TextSelection.between(he, fe);
            ee(B.tr.setSelection(Ie));
          },
          setNodeSelection(K) {
            if (!g.current) return;
            let { state: C, dispatch: B } = g.current;
            try {
              let ee = NodeSelection.create(C.doc, K);
              B(C.tr.setSelection(ee));
            } catch {
              this.setSelection(K);
            }
          },
          setCellSelection(K, C) {
            ...
            let { state: B, dispatch: ee } = g.current;
            try {
              let he = CellSelection.create(B.doc, K, C);
              ee(B.tr.setSelection(he));
            } catch {
      ...
      jsx('div', {
        ...
        style: { ...cg, width: a > 0 ? `${a}px` : void 0 },
      })
    );
  }),
  Md = memo(ug);
var gg = '#000',
  hg = 'rgba(66, 133, 244, 0.3)',
  bg = 2,
  yg = 530,
  xg = {
    position: 'absolute',
  ...
  wg = ({ position: e, color: t, width: n, blinkInterval: o, isFocused: r }) => {
    let [i, a] = useState(r),
      s = useRef(null);
    return (
      useEffect(
      ...
      jsx('div', { style: Sg(e, t, n, i), 'data-testid': 'caret' })
    );
  },
  Cg = ({ rect: e, color: t, index: n }) =>
    jsx('div', {
      style: kg(e, t),
  ...
  Id = ({
    ...
    return jsxs('div', {
      ...
      children: [
        l &&
          e.map((d, c) =>
            jsx(Cg, { rect: d, color: i, index: c }, `sel-${d.pageIndex}-${d.x}-${d.y}-${c}`)
          ),
        u && t && jsx(wg, { position: t, color: r, width: a, blinkInterval: s, isFocused: n }),
      ],
    });
  };
var Ja = 10,
  ...
  Mg = { nw: 'nw-resize', ne: 'ne-resize', se: 'se-resize', sw: 'sw-resize' };
...
function Dd({
      ...
      (P, I) => {
        ...
        let N = (G) => {
            let xe = K.current,
              ...
              D = !G.shiftKey,
              ie = Eg(P, v, z, y, M, D);
            ((U = Math.round(ie.width)),
              (Q = Math.round(ie.height)),
              m(U),
              g(Q),
              w((de) => {
                if (!de) return de;
                let ue = { ...de };
                return (
                  P.includes('w') && (ue.left = de.left + (de.width - ie.width)),
                  P.includes('n') && (ue.top = de.top + (de.height - ie.height)),
                  (ue.width = ie.width),
                  (ue.height = ie.height),
                  ue
                );
          ...
          j = () => {
            (window.removeEventListener('mousemove', N),
              ...
              d(false));
            let G = O.current;
            (G && R.current?.(G.pmPos, U, Q), V.current?.());
          };
        (window.addEventListener('mousemove', N), window.addEventListener('mouseup', j));
      },
      [e, S]
    ),
    ee = useCallback(
        ...
        let I = 4,
          ...
          Q = (N) => {
            if (
              ...
              let j = O.current;
              (j && $.current?.(j.pmPos, N.clientX, N.clientY), W.current?.());
            }
          };
  ...
  if (!!!(e && S && n))
    return jsx('div', {
      ref: E,
      style: { ...Ld, visibility: 'hidden' },
      className: 'image-selection-overlay',
    });
  let { left: fe, top: Ie, width: Te, height: Re } = S;
  return jsxs('div', {
    ref: E,
    style: Ld,
    className: 'image-selection-overlay',
    children: [
      jsx('div', {
        style: { ...vg, left: fe - Wo, top: Ie - Wo, width: Te + Wo * 2, height: Re + Wo * 2 },
      }),
      jsx('div', {
        style: {
          position: 'absolute',
      ...
      jsx(Pi, { handle: 'nw', style: { left: fe - Tn, top: Ie - Tn }, onMouseDown: B }),
      jsx(Pi, { handle: 'ne', style: { left: fe + Te - Tn, top: Ie - Tn }, onMouseDown: B }),
      jsx(Pi, { handle: 'se', style: { left: fe + Te - Tn, top: Ie + Re - Tn }, onMouseDown: B }),
      jsx(Pi, { handle: 'sw', style: { left: fe - Tn, top: Ie + Re - Tn }, onMouseDown: B }),
      u &&
        jsxs('div', {
          style: { ...Pg, left: fe + Te / 2, top: Ie + Re + 12 },
          children: [f, ' \xD7 ', b],
        }),
    ],
  });
}
function Pi({ handle: e, style: t, onMouseDown: n }) {
  return jsx('div', {
    style: { ...Rg, ...t, cursor: Mg[e] },
    onMouseDown: (o) => n(e, o),
    'data-handle': e,
  });
}
...
function Qa(e) {
  ...
  let u = Ig(t.w, n.left, n.right, o);
  function d(E) {
    return n.left + E * (u + o.gap);
  }
  function c() {
    let E = r.length + 1,
      R = e.footnoteReservedHeights?.get(E) ?? 0,
      _ = s - R,
      V = {
        number: E,
        fragments: [],
        margins: { ...n },
        size: { ...t },
        footnoteReservedHeight: R > 0 ? R : void 0,
      },
      $ = {
        page: V,
        cursorY: a,
    ...
    return (r.push(V), i.push($), e.onNewPage && e.onNewPage($), $);
  }
  function p() {
  ...
  function g(E) {
    let R = p(),
      ...
      V = R.contentBottom - R.topMargin;
    if (_ > V) return (R.cursorY !== R.topMargin && (R = b(R)), R);
    for (; !m(_, R); ) R = b(R);
    return R;
  }
  function S(E, R, _ = 0, V = 0) {
    let $ = Math.max(_, p().trailingSpacing),
      L = $ + R,
      W = g(L),
...
var Fg = { w: 816, h: 1056 },
  co = { top: 96, right: 96, bottom: 96, left: 96 };
function Hd(e) {
...
function Mi(e, t, n = {}) {
  ...
  let o = n.pageSize ?? Fg,
    r = {
      top: n.margins?.top ?? co.top,
      right: n.margins?.right ?? co.right,
      bottom: n.margins?.bottom ?? co.bottom,
      left: n.margins?.left ?? co.left,
      header: n.margins?.header ?? n.margins?.top ?? co.top,
      footer: n.margins?.footer ?? n.margins?.bottom ?? co.bottom,
    };
  ...
  for (let d = 0; d < e.length; d++) {
    ...
    switch (c.kind) {
      case 'paragraph':
        ...
        break;
      case 'table':
        c.floating ? Dg(c, p, s, a) : Ag(c, p, s);
        break;
      case 'image':
        Hg(c, p, s);
        break;
  ...
  return (
    e.forEach((a, s) => {
      ...
      if (a.isText && a.text) {
        ...
        let d = { kind: 'tab', ...Ei(a.marks, i), pmStart: l, pmEnd: l + a.nodeSize };
        o.push(d);
      } else if (a.type.name === 'image') {
        let u = a.attrs,
          d = rs(u.width || 100, u.height || 100, n.pageContentHeight),
          c = {
            kind: 'image',
            src: u.src,
            width: d.width,
        ...
        a.forEach((d, c) => {
          ...
          if (d.isText && d.text) {
            ...
            let m = { kind: 'tab', ...Ei(d.marks, i), pmStart: p, pmEnd: p + d.nodeSize };
            o.push(m);
          } else if (d.type.name === 'image') {
            let f = d.attrs,
              m = rs(f.width || 100, f.height || 100, n.pageContentHeight),
              b = {
                kind: 'image',
                src: f.src,
                width: m.width,
...
function jg(e) {
  ...
  (e.shading?.fill?.rgb && (t.shading = `#${e.shading.fill.rgb}`),
    e.tabs &&
      e.tabs.length > 0 &&
      (t.tabs = e.tabs.map((a) => ({ val: Ug(a.alignment), pos: a.position, leader: a.leader }))),
...
function zd(e, t, n) {
  let o = e.attrs,
    r = Vg(e, t, n),
    i = jg(o);
  return { kind: 'paragraph', id: uo(), runs: r, attrs: i, pmStart: t, pmEnd: t + e.nodeSize };
}
function Gg(e) {
...
function Yg(e) {
  ...
  for (let r of o) {
    ...
    let a = { style: Kg[i.style] || 'solid' };
    (i.color?.rgb && (a.color = `#${i.color.rgb}`), i.size && (a.width = Gg(i.size)), (n[r] = a));
  }
  return Object.keys(n).length > 0 ? n : void 0;
}
function qg(e, t, n) {
  ...
  e.forEach((l) => {
    (l.type.name === 'paragraph'
      ? o.push(zd(l, r, n))
      : l.type.name === 'table' && o.push(Od(l, r, n)),
      (r += l.nodeSize));
  });
  let i = e.attrs,
    a = i.margins,
...
function Od(e, t, n) {
  let o = [],
    r = t + 1;
  e.forEach((p) => {
    (p.type.name === 'tableRow' && o.push(Xg(p, r, n)), (r += p.nodeSize));
  });
...
function Zg(e, t, n) {
  let o = e.attrs,
    ...
    a = rs(o.width || 100, o.height || 100, n);
  return {
    kind: 'image',
    id: uo(),
    src: o.src,
...
function $d(e, t = {}) {
  let n = { ...t, defaultFont: t.defaultFont ?? Og, defaultSize: t.defaultSize ?? $g },
    o = [],
    r = 0,
    i = new Map();
  return (
    e.forEach((a, s) => {
      ...
      switch (a.type.name) {
        ...
        case 'table':
          ...
          break;
        case 'image':
          o.push(Zg(a, l, n.pageContentHeight));
          break;
        case 'horizontalRule':
        case 'pageBreak': {
          let u = { kind: 'pageBreak', id: uo(), pmStart: l, pmEnd: l + a.nodeSize };
          o.push(u);
          break;
        }
      }
...
function _d(e, t, n) {
  let o = d$2({ fontSize: e, fontFamily: n ?? _o });
  return Ud(e, t, o);
}
function Vd(e) {
  return e.kind === 'text';
...
function Vo(e, t, n) {
  ...
  if (o.length === 0) {
    let L = r?.defaultFontSize ?? pn,
      W = r?.defaultFontFamily ?? _o,
      O = _d(L, i, W);
    return (
      w.push({ fromRun: 0, fromChar: 0, toRun: 0, toChar: 0, width: 0, ...O }),
      { kind: 'paragraph', lines: w, totalHeight: O.lineHeight }
    );
  }
  if (o.length === 1 && Vd(o[0]) && oh(o[0])) {
    let L = o[0],
      W = L.fontSize ?? r?.defaultFontSize ?? pn,
      O = L.fontFamily ?? r?.defaultFontFamily ?? _o,
      K = _d(W, i, O);
    return (
  ...
  let k = {
    ...
    E = () => {
      let L = Ud(k.maxFontSize, i, k.maxFontMetrics),
        W = { ...L };
      k.maxImageHeightPx > W.lineHeight &&
        ((W.lineHeight = k.maxImageHeightPx),
        (W.ascent = k.maxImageHeightPx * 0.8),
        (W.descent = k.maxImageHeightPx * 0.2));
      let O = {
        fromRun: k.fromRun,
        fromChar: k.fromChar,
        toRun: k.toRun,
      ...
      (k.leftOffset > 0 && (O.leftOffset = k.leftOffset),
        k.rightOffset > 0 && (O.rightOffset = k.rightOffset),
        w.push(O),
        (m += L.lineHeight));
    },
    R = (L, W) => {
    ...
    _ = (L) => {
      let W = L.fontSize ?? pn;
      (!k.maxFontMetrics || W > k.maxFontSize) &&
        ((k.maxFontSize = W), (k.maxFontMetrics = d$2(L)));
    };
  for (let L = 0; L < o.length; L++) {
    let W = o[L];
    if (th(W)) {
      ((k.toRun = L), (k.toChar = 0), R(L + 1, 0));
      continue;
    }
    if (Qg(W)) {
      let O = Wd(W);
      _(O);
      let K = W.width ?? ih;
      (k.width + K > k.availableWidth + po && (R(L, 0), _(O)),
        (k.width += K),
        (k.toRun = L),
        (k.toChar = 1));
      continue;
    ...
    if (eh(W)) {
      let O = W.wrapType,
        K = W.displayMode === 'float' || (O && ['square', 'tight', 'through'].includes(O));
      if (W.position && K) {
        ((k.toRun = L), (k.toChar = 1));
        continue;
      }
      if (O === 'topAndBottom' || W.displayMode === 'block') {
        k.width > 0 && R(L, 0);
        let ee = W.height,
          he = W.distTop ?? 6,
          fe = W.distBottom ?? 6;
        ((k.toRun = L), (k.toChar = 1), (k.maxImageHeightPx = ee + he + fe), R(L + 1, 0));
        continue;
      }
      let C = W.width,
        B = W.height;
      (B > k.maxImageHeightPx && (k.maxImageHeightPx = B),
        k.width + C > k.availableWidth + po && R(L, 0),
        (k.width += C),
        (k.toRun = L),
        (k.toChar = 1));
      continue;
    ...
    if (nh(W)) {
      ...
      _(K);
      let C = e$2(O, K);
      (k.width > 0 && k.width + C > k.availableWidth + po && (R(L, 0), _(K)),
        (k.width += C),
        (k.toRun = L),
        (k.toChar = 1));
      continue;
    ...
    if (Vd(W)) {
      ...
      for (; ee < K.length; ) {
        ...
        let fe = K.slice(ee, he),
          Ie = e$2(fe, C);
        if (Ie > k.availableWidth + po) {
          k.width > 0 && (R(L, ee), _(C));
          let { charWidths: Te } = f$2(fe, C),
            Re = 0;
          for (; Re < fe.length; ) {
            let P = 0,
              I = Re;
            for (; I < fe.length; ) {
              let y = Te[I] ?? 0;
              if (P + y > k.availableWidth + po) break;
              ((P += y), (I += 1));
            }
            (I === Re && ((I = Math.min(fe.length, Re + 1)), (P = Te[Re] ?? 0)),
              (k.width += P),
              (k.toRun = L),
              (k.toChar = ee + I),
              (Re = I),
              Re < fe.length && (R(L, ee + Re), _(C)));
          }
          ee = he;
          continue;
        }
        (k.width > 0 && k.width + Ie > k.availableWidth + po && (R(L, ee), _(C)),
          (k.width += Ie),
          (k.toRun = L),
          (k.toChar = he),
          (ee = he));
...
function ss(e, t, n) {
  let o = is(e);
  (Rn.set(o, { measure: n, maxWidth: t }), dh());
}
function Yd() {
  Rn.clear();
}
...
function qd(e) {
  ...
  let p = s.attrs?.indent,
    ...
    g = r - f,
    { charOffset: S, pmPosition: w } = gh(s, d, g, b);
...
function Xd(e, t, n) {
  ...
  for (let c of Array.from(s)) {
    let p = c,
      f = p.getBoundingClientRect();
    if (t >= f.left && t <= f.right) return ds(p, t);
...
function bh(e, t, n, o, r) {
  ...
  for (let f of Array.from(u)) {
    let m = f,
      b = m.getBoundingClientRect();
    if (n >= b.left && n <= b.right) return ds(m, n);
...
function jo(e, t, n = {}) {
  ...
  if (yh(e)) return kh(e, t, o);
  if (xh(e)) return wh(e, t, o);
  if (Sh(e)) return Ch(e, t, o);
  let r = o.createElement('div');
  ((r.className = Pn.fragment), Fi(r));
  let i = e;
  return (
...
function gs(e, t) {
  ...
  if (((n.className = `${Ot.run} ${Ot.text}`), Rh(n, e), fo(n, e.pmStart, e.pmEnd), e.hyperlink)) {
...
function ou(e, t) {
  ...
  return ((n.className = `${Ot.run} ${Ot.lineBreak}`), fo(n, e.pmStart, e.pmEnd), n);
}
function ru(e, t, n) {
  let o = e.fallback ?? '';
  switch (e.fieldType) {
...
function Eh(e, t, n) {
  if (Go(e)) return gs(e, t);
  if (Li(e)) return tu(e, t, 48, void 0);
  if (ps(e)) return nu(e, t);
  if (fs(e)) return ou(e, t);
  if (ms(e) && n) return ru(e, t, n);
...
function au(e, t, n, o, r) {
  ...
  let u = a.some((b) => Go(b) && b.highlight);
  i.style.overflow = u ? 'hidden' : 'visible';
  let d = a.some(Li),
    c,
    p = Lh(o);
  if (d) {
    let b = r?.tabStops?.map(Ih),
  ...
  for (let b = 0; b < a.length; b++) {
    let g = a[b];
    if (Li(g) && c) {
      let S = Fh(a, b, r?.context),
        w = Jd(f, c, S, p),
        k = tu(g, o, w.width, w.leader);
      (i.appendChild(k), (f += w.width));
    } else if (Go(g)) {
      let S = gs(g, o);
      if (g.highlight) {
        let R = (g.fontSize ? (g.fontSize * 96) / 72 : 14.67) * 1.2,
          _ = Math.max(0, t.lineHeight - R);
        if (_ > 0) {
          let V = _ / 2;
      ...
      let S = ou(g, o);
      i.appendChild(S);
    } else if (ms(g) && r?.context) {
      let S = ru(g, o, r.context);
      i.appendChild(S);
      let w = g.fallback ?? '';
      g.fieldType === 'PAGE'
...
function eu(e, t) {
  return (!e && !t) || !e || !t
    ? false
    : Uo(e.top, t.top) &&
        Uo(e.bottom, t.bottom) &&
        Uo(e.left, t.left) &&
        Uo(e.right, t.right) &&
        Uo(e.between, t.between);
}
function jn(e, t, n, o, r = {}) {
  ...
  if (p) {
    ...
    let E = ($) => `${$.width}px ${k($.style)} ${$.color}`,
      R = eu(r.prevBorders, p),
      _ = eu(p, r.nextBorders);
    (R && p.between
      ? (a.style.borderTop = E(p.between))
      : p.top && !R && (a.style.borderTop = E(p.top)),
  ...
  for (let k = 0; k < s.length; k++) {
    ...
    if (V) {
      ...
      B && u?.hanging ? (W = f + u.hanging) : ee && u?.firstLine && (W = f - u.firstLine);
    }
    let O = au(t, E, l, i, {
      availableWidth: W - $ - L,
      isLastLine: _,
    ...
    ($ > 0 && (O.style.marginLeft = `${$}px`),
      L > 0 && (O.style.marginRight = `${L}px`),
      (w += E.lineHeight));
    ...
    if (
      (V
        ? d > 0 && K
          ? ((O.style.paddingLeft = `${d}px`), (O.style.textIndent = `-${u.hanging}px`))
          : d > 0 && C
            ? ((O.style.paddingLeft = `${d}px`), (O.style.textIndent = `${u.firstLine}px`))
            : d > 0
              ? (O.style.paddingLeft = `${d}px`)
              : C && (O.style.textIndent = `${u.firstLine}px`)
        : d > 0
          ? (O.style.paddingLeft = `${d}px`)
          : K && (O.style.paddingLeft = `${u.hanging}px`),
      c > 0 && (O.style.paddingRight = `${c}px`),
      V && t.attrs?.listMarker)
    ) {
      let B = Math.max(0, d - (u?.hanging ?? 0));
      ((O.style.paddingLeft = `${B}px`), (O.style.textIndent = '0'));
      let ee = Bh(t.attrs.listMarker, u, i);
      O.insertBefore(ee, O.firstChild);
    }
    a.appendChild(O);
  }
  return a;
...
function Dh(e, t, n, o) {
  ...
  for (let u = 0; u < e.rows.length; u++) {
    let d = e.rows[u],
      c = t.rows[u];
    if (!d || !c) continue;
    let p = su(d, c, u, l, t.columnWidths, e.rows.length, n, o, s, i);
...
function Hh(e, t, n, o, r, i, a) {
  ...
  if (
    ...
    e.borders &&
      (r.isFirstRow && Bi(s, 'top', e.borders.top),
      Bi(s, 'right', e.borders.right),
      Bi(s, 'bottom', e.borders.bottom),
      r.isFirstCol && Bi(s, 'left', e.borders.left)),
...
function su(e, t, n, o, r, i, a, s, l, u) {
  ...
  for (let m = 0; m < e.cells.length; m++) {
    ...
    let E = n === 0,
      _ = f === 0;
    f + S >= r.length;
    let $ = Hh(b, g, p, k, { isFirstRow: E, isFirstCol: _ }, a, s);
    if (
...
function Ai(e, t, n, o, r = {}) {
  ...
  for (let f = e.fromRow; f < e.toRow; f++) {
    let m = t.rows[f],
      b = n.rows[f];
    if (!m || !b) continue;
    let g = su(m, b, f, c, n.columnWidths, t.rows.length, o, i, d, l);
...
var hs = { image: 'layout-image', imageAnchored: 'layout-image-anchored' };
function Di(e, t, n, o, r = {}) {
  let i = r.document ?? document,
    a = i.createElement('div');
  ((a.className = hs.image),
    e.isAnchored && a.classList.add(hs.imageAnchored),
    (a.style.position = 'absolute'),
    (a.style.width = `${e.width}px`),
    (a.style.height = `${e.height}px`),
    (a.style.overflow = 'hidden'),
      ...
      m.align === 'right'
        ...
        : m.align === 'left'
          ...
          : m.align === 'center'
            ...
            : m.posOffset !== void 0 && ((p = Oi(m.posOffset)), (c = p > n / 2 ? 'right' : 'left'));
    ...
    d &&
      (d.align === 'right'
        ...
        : d.align === 'center'
          ...
          : d.posOffset !== void 0
            ? (u.style.left = `${Oi(d.posOffset)}px`)
            : (u.style.left = '0'));
    ...
    if (c) {
      ...
      c.align === 'bottom'
        ...
        : c.align === 'center'
          ...
          : c.posOffset !== void 0
            ? (u.style.top = `${p + Oi(c.posOffset)}px`)
            : (u.style.top = `${p}px`);
...
function $i(e, t, n = {}) {
  ...
  ((r.className = n.pageClassName ?? Mn.page),
    ...
    Ni(r, e.size.w, e.size.h, n));
  let i = o.createElement('div');
  ((i.className = Mn.content), Nh(i, e));
    ...
    if (n.blockLookup && f.blockId) {
      ...
      if (
        ...
        f.kind === 'table' && S?.block.kind === 'table' && S?.measure.kind === 'table'
          ? ((m = Ai(f, S.block, S.measure, b, { document: o })), (c = void 0))
          : f.kind === 'image' && S?.block.kind === 'image' && S?.measure.kind === 'image'
            ? ((m = Di(f, S.block, S.measure, b, { document: o })), (c = void 0))
            : ((m = jo(f, b, { document: o })), (c = void 0));
    } else ((m = jo(f, b, { document: o })), (c = void 0));
    (zh(m, f, { left: e.margins.left, top: e.margins.top }), i.appendChild(m));
  }
  if (n.footnoteArea && n.footnoteArea.length > 0) {
    let p = Vh(n.footnoteArea, a, o);
  ...
  r.appendChild(i);
  {
    ...
    if (
      ...
      let k = lu(n.headerContent, { ...t, section: 'header', contentWidth: m }, n);
      w.appendChild(k);
    }
    r.appendChild(w);
  }
  {
    ...
    if (
      ...
      let k = lu(n.footerContent, { ...t, section: 'footer', contentWidth: m }, n);
      w.appendChild(k);
    }
    r.appendChild(w);
  }
...
function ys(e, t, n = {}) {
  ...
  if (a && a.optionsHash === s && l) {
    let g = a.pageStates,
      ...
      w = i.__pageObserver,
      k = [];
    for (let _ of e) k.push(cu(_));
    let E = a.totalPages !== o,
      R = Math.min(g.length, e.length);
    for (let _ = 0; _ < R; _++) {
      let V = g[_],
        $ = k[_];
      if (V.fingerprint === $ && !E) {
        let O = S.get(V.element);
        O && (O.page = e[_]);
        continue;
      }
      let L = V.element,
        W = S.get(L);
      (W && ((W.page = e[_]), W.rendered && Kh(L, S, o, n)),
        (V.fingerprint = $),
        Ni(L, e[_].size.w, e[_].size.h, n),
    ...
    if (e.length > g.length) {
      ...
      for (let V = g.length; V < e.length; V++) {
        ...
        ((L.className = n.pageClassName ?? Mn.page),
          ...
          Ni(L, $.size.w, $.size.h, n),
          t.appendChild(L),
          g.push({ element: L, fingerprint: k[V] }),
          S.set(L, { page: $, index: V, rendered: false }),
          w && w.observe(L));
      }
    }
    if (e.length < g.length) {
  ...
  for (let g = 0; g < e.length; g++) {
    ...
    if ((p.push(cu(S)), l)) {
      ...
      ((k.className = n.pageClassName ?? Mn.page),
        ...
        Ni(k, S.size.w, S.size.h, n),
        t.appendChild(k),
        c.push(k));
    } else {
      let { context: w, pageOptions: k } = bs(S, o, n),
        E = $i(S, w, k);
  ...
  let m = new IntersectionObserver(
    (g) => {
        ...
        if (W && $.isIntersecting) {
          ...
          for (let O = -Hi; O <= Hi; O++) {
            ...
            K >= 0 &&
              ...
              K !== W.index &&
              zi(S.pageStates[K].element, E, k, w);
          }
        }
      }
      ...
      for (let [$, L] of E) {
        ...
        W.bottom > -_ && W.top < R + _ && V.add(L.index);
      }
      for (let [$, L] of E) {
        if (!L.rendered) continue;
        let W = false;
        for (let O of V)
          if (Math.abs(L.index - O) <= Hi + 1) {
            W = true;
            break;
          }
        !W && V.size > 0 && Yh($, E);
  ...
  let b = Math.min(e.length, Hi + 3);
  for (let g = 0; g < b; g++) zi(c[g], f, o, n);
}
function zi(e, t, n, o) {
  let r = t.get(e);
  if (!r || r.rendered) return;
  let { context: i, pageOptions: a } = bs(r.page, n, o),
    s = $i(r.page, i, a);
...
function Kh(e, t, n, o) {
  ...
  if (!r) return;
  let { context: i, pageOptions: a } = bs(r.page, n, o),
    l = $i(r.page, i, a).querySelector(`.${Mn.content}`),
    u = e.querySelector(`.${Mn.content}`);
  l && u ? e.replaceChild(l, u) : ((e.innerHTML = ''), (r.rendered = false), zi(e, t, n, o));
}
function Yh(e, t) {
  let n = t.get(e);
...
var Wi = class {
  ...
  renderFragmentWithLookup(t, n) {
    ...
    if (t.kind === 'table' && o) {
      let r = o.block,
        i = o.measure;
      return Ai(t, r, i, n, { document: this.doc });
    }
    if (t.kind === 'image' && o) {
      let r = o.block,
        i = o.measure;
      return Di(t, r, i, n, { document: this.doc });
    }
    return jo(t, n, { document: this.doc });
  }
...
var fn,
  ...
  _i = class {
    ...
    onLayoutComplete(t) {
      (g(this, En, t), g(this, In, false), h(this, Un, du).call(this));
    }
    isSafeToRender() {
    ...
    requestRender() {
      this.isSafeToRender()
        ? h(this, Un, xs).call(this)
        : g(this, mn, () => h(this, Un, xs).call(this));
    }
    onRender(t) {
      return (
        e(this, mo).add(t),
...
function pu({ pagesContainerRef: e }) {
  let t = useRef(null),
    n = useRef(-1),
    o = useCallback(
      (s) => {
      ...
      for (let f of Array.from(u)) {
        ...
        if (l >= b.left && l <= b.right) {
          ...
          let R = 0,
            _ = k.length;
          for (; R < _; ) {
            let V = Math.floor((R + _) / 2),
              $ = E.createRange();
            ($.setStart(k, V),
              $.setEnd(k, V),
              l < $.getBoundingClientRect().left ? (_ = V) : (R = V + 1));
          }
          if (R > 0 && R <= k.length) {
            let V = E.createRange();
            (V.setStart(k, R - 1), V.setEnd(k, R - 1));
            let $ = V.getBoundingClientRect().left;
            (V.setStart(k, Math.min(R, k.length)), V.setEnd(k, Math.min(R, k.length)));
            let L = V.getBoundingClientRect().left;
            if (Math.abs(l - $) < Math.abs(l - L)) return g + (R - 1);
          }
          return g + Math.min(R, S - g);
        }
      }
      let d = null,
        c = 1 / 0;
...
function Xh(e, t, n) {
  ...
  for (let a = 0; a < e.content.length; a++) {
    let s = e.content[a],
      l = [];
    if (a === 0) {
      let d = { kind: 'text', text: `${t}  `, fontSize: Ui, superscript: true };
      l.push(d);
    }
    ...
    l.length === 0 && l.push({ kind: 'text', text: '', fontSize: Ui });
  ...
  o.length === 0 &&
    o.push({
      kind: 'paragraph',
      id: `fn-${e.id}-empty`,
      runs: [{ kind: 'text', text: '', fontSize: Ui }],
    });
  let r = [];
  for (let a of o)
    if (a.kind === 'paragraph') {
...
var eb = 816,
  ku = 1056,
  Gi = { top: 96, right: 96, bottom: 96, left: 96 },
  tb = 24,
  nb = [],
  ob = {
    position: 'relative',
  ...
  ib = { position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center' },
  ab = {
    position: 'absolute',
    top: 0,
    left: 0,
...
function sb(e) {
  return { w: e?.pageWidth ? sn(e.pageWidth) : eb, h: e?.pageHeight ? sn(e.pageHeight) : ku };
}
function lb(e) {
  let t = e?.marginTop ? sn(e.marginTop) : Gi.top,
    n = e?.marginBottom ? sn(e.marginBottom) : Gi.bottom;
  return {
    top: t,
    right: e?.marginRight ? sn(e.marginRight) : Gi.right,
    bottom: n,
    left: e?.marginLeft ? sn(e.marginLeft) : Gi.left,
...
function wu(e, t) {
  ...
  let l = e.rows.map((c, p) => {
    ...
    return {
      cells: c.cells.map((b) => {
        let g = b.colSpan ?? 1,
          S = 0;
        for (let R = 0; R < g && f + R < i.length; R++) S += i[f + R] ?? 0;
        for (S === 0 && (S = b.width ?? 100), f += g; m.has(f); ) f++;
        let w = b.padding?.left ?? 7,
          k = b.padding?.right ?? 7,
          E = Math.max(1, S - w - k);
        return {
          blocks: b.blocks.map((R) => Cu(R, E)),
          width: S,
          height: 0,
          colSpan: b.colSpan,
          rowSpan: b.rowSpan,
  ...
  for (let c = 0; c < l.length; c++) {
    ...
    for (let w = 0; w < p.cells.length; w++) {
      let k = p.cells[w],
        E = f?.[w];
      k.height = k.blocks.reduce((V, $) => ('totalHeight' in $ ? V + $.totalHeight : V), 0);
      let R = E?.padding?.top ?? 1,
        _ = E?.padding?.bottom ?? 1;
      ((k.height += R + _), (m = Math.max(m, k.height)));
    }
    let b = e.rows[c],
      g = b?.height,
      S = b?.heightRule;
    ...
    for (let a of i.runs) {
      ...
      if (u?.vertical) {
        ...
        S.align === 'top' && S.relativeTo === 'margin'
          ? (l = 0)
          : S.posOffset !== void 0 && (l = yu(S.posOffset));
      }
      let m = l + s.height,
        b = 0,
        g = 0;
...
function Cu(e, t, n, o) {
  switch (e.kind) {
    case 'paragraph': {
      ...
      let i = Vo(r, t, { floatingZones: n, paragraphYOffset: o ?? 0 });
      return ((!n || n.length === 0) && ss(r, t, i), i);
    }
    case 'table':
      return wu(e, t);
    case 'image': {
...
function xu(e, t) {
  ...
  let o = n.map((a) => {
    ...
    r = Tu(o, t),
    i = r.reduce((a, s) => (s.kind === 'paragraph' ? a + s.totalHeight : a), 0);
...
var fb = forwardRef(function (t, n) {
    let {
        document: o,
        styles: r,
        theme: i$1,
      ...
      C = useRef(null),
      B = useRef(null),
      { handlePMKeyDown: ee } = pu({ pagesContainerRef: K }),
      he = useRef(f),
      fe = useRef(p),
      Ie = useRef(g),
      Te = useRef(S);
    ((he.current = f), (fe.current = p), (Ie.current = g), (Te.current = S));
    let [Re, P] = useState(null),
      ...
      Wt = useMemo(() => new Wi({ pageGap: d, showShadow: true, pageBackground: '#fff' }), [d]);
    B.current = Wt;
    let Pt = useCallback(
        (T) => {
          ...
          try {
            let X = performance.now(),
              ...
              Y = $d(T.doc, { theme: i$1, pageContentHeight: te }),
              Z = performance.now() - X;
            (Z > 500 &&
              console.warn(
                `[PagedEditor] toFlowBlocks took ${Math.round(Z)}ms (${Y.length} blocks)`
              ),
              y(Y),
              (X = performance.now()));
            let J = Tu(Y, Lt);
            ((Z = performance.now() - X),
              Z > 1e3 &&
                console.warn(
                  `[PagedEditor] measureBlocks took ${Math.round(Z)}ms (${Y.length} blocks)`
                ),
              F(J));
            let ce = fu(Y),
            ...
            let zt,
              ...
              xo = new Map();
            if (we) {
              let bn = Mi(Y, J, { pageSize: Oe, margins: Dt });
              ((Hn = Ss(bn.pages, ce)), (xo = mu(o.package.footnotes, ce, Lt)));
              let Qt = gu(Hn, xo);
              if (Qt.size > 0) {
                ((zt = Mi(Y, J, { pageSize: Oe, margins: Dt, footnoteReservedHeights: Qt })),
                  (Hn = Ss(zt.pages, ce)));
                for (let [nt, Nn] of Hn) {
                  let zn = zt.pages.find((oa) => oa.number === nt);
                  zn && (zn.footnoteIds = Nn);
                }
              } else zt = bn;
            } else zt = Mi(Y, J, { pageSize: Oe, margins: Dt });
            if (
              ((Z = performance.now() - X),
              Z > 500 &&
                console.warn(
                  `[PagedEditor] layoutDocument took ${Math.round(Z)}ms (${zt.pages.length} pages)`
                ),
              P(zt),
              K.current && B.current)
            ) {
              ...
              let Qt = we ? pb(Hn, xo, o) : void 0;
              if (
                (ys(zt.pages, K.current, {
                  pageGap: d,
                  showShadow: !0,
                  pageBackground: '#fff',
                  blockLookup: bn,
                ...
                (Z = performance.now() - X),
                Z > 500 && console.warn(`[PagedEditor] renderPages took ${Math.round(Z)}ms`),
                S)
              ) {
                let nt = hu(K.current, c);
                S(nt);
              }
            }
            let So = performance.now() - ne;
            So > 2e3 &&
              console.warn(
                `[PagedEditor] Layout pipeline took ${Math.round(So)}ms total (${Y.length} blocks, ${J.length} measures)`
              );
          } catch (X) {
    ...
    let Jn = useCallback((T, ne = 1) => {
        ...
        for (let Z of Array.from(te)) {
          let J = Z,
            ce = Number(J.dataset.pmStart),
            we = Number(J.dataset.pmEnd);
          if (J.classList.contains('layout-run-tab')) {
            if (T >= ce && T < we) {
              let be = J.getBoundingClientRect(),
                Ce = J.closest('.layout-page'),
                Fe = Ce ? Number(Ce.dataset.pageNumber) - 1 : 0,
          ...
          if (T >= ce && T <= we && Z.firstChild?.nodeType === Node.TEXT_NODE) {
            let be = Z.firstChild,
              Ce = Math.min(T - ce, be.length),
              Fe = J.ownerDocument;
            if (!Fe) continue;
            let Le = Fe.createRange();
            (Le.setStart(be, Ce), Le.setEnd(be, Ce));
            let Ye = Le.getBoundingClientRect(),
              Xe = J.closest('.layout-page'),
              xt = Xe ? Number(Xe.dataset.pageNumber) - 1 : 0,
              dt = J.closest('.layout-line'),
        ...
        for (let Z of Array.from(Y)) {
          let J = Z.closest('.layout-paragraph');
          if (!J) continue;
          let ce = Number(J.dataset.pmStart),
            we = Number(J.dataset.pmEnd);
          if (T >= ce && T <= we) {
            let be = Z.getBoundingClientRect(),
              Ce = J.closest('.layout-page'),
              Fe = Ce ? Number(Ce.dataset.pageNumber) - 1 : 0,
              Le = Z.closest('.layout-line'),
              Ye = Le ? Le.offsetHeight : 16;
            return {
              x: (be.left - X.left) / ne,
              y: (be.top - X.top) / ne,
      ...
      Bt = useCallback(
        (T) => {
          ...
          if ((he.current?.(ne, q), K.current)) {
            ...
            for (let Z of Array.from(X)) Z.classList.remove('layout-table-cell-selected');
            let te = T.selection;
            if ('$anchorCell' in te && typeof te.forEachCell == 'function') {
              let Z = [];
              te.forEachCell((ce, we) => {
                Z.push([we, we + ce.nodeSize]);
              });
              let J = K.current.querySelectorAll('.layout-table-cell');
              for (let ce of Array.from(J)) {
                let we = ce,
                  be = we.dataset.pmStart;
                if (be !== void 0) {
                  let Ce = Number(be);
                  for (let [Fe, Le] of Z)
                    if (Ce >= Fe && Ce < Le) {
          ...
          if (!(!Re || I.length === 0))
            if (ne === q) {
              ...
              if (X) G(X);
                ...
                if (te && Y) {
                  let Z = te.getBoundingClientRect(),
                    J = Y.getBoundingClientRect(),
                    ce = l(Re, I, M, ne);
                  G(
                    ce
                      ? { ...ce, x: ce.x + (J.left - Z.left) / c, y: ce.y + (J.top - Z.top) / c }
                      : null
                  );
                } else G(null);
              }
              ...
              if (X && K.current) {
                let te = X.getBoundingClientRect(),
                  Y = [],
                  Z = K.current.querySelectorAll('span[data-pm-start][data-pm-end]');
                for (let J of Array.from(Z)) {
                  let ce = J,
                    we = Number(ce.dataset.pmStart);
                  if (Number(ce.dataset.pmEnd) > ne && we < q) {
                    ...
                    if (J.firstChild?.nodeType !== Node.TEXT_NODE) continue;
                    let Ce = J.firstChild,
                      Fe = ce.ownerDocument;
                    if (!Fe) continue;
                    let Le = Math.max(0, ne - we),
                      Ye = Math.min(Ce.length, q - we);
                    if (Le < Ye) {
                      let Xe = Fe.createRange();
                      (Xe.setStart(Ce, Le), Xe.setEnd(Ce, Ye));
                ...
                if (Y.length > 0) N(Y);
                  ...
                  let J = K.current.querySelector('.layout-page');
                  if (J) {
                    let ce = J.getBoundingClientRect(),
                      we = (ce.left - te.left) / c,
                      be = (ce.top - te.top) / c,
                      Fe = k(Re, I, M, ne, q).map((Le) => ({ ...Le, x: Le.x + we, y: Le.y + be }));
                    N(Fe);
                  } else N([]);
                }
              } else N([]);
      ...
      At = useCallback(
        (T, ne) => {
          ...
          let Z = (T - Y.left) / c,
            ...
            ce = Re.pages[te];
          if (!ce) return null;
          let we = { pageIndex: te, page: ce, pageY: J },
            be = i(we, I, M, { x: Z, y: J });
          if (!be) return null;
          if (be.fragment.kind === 'table') {
            let Ce = j(we, I, M, { x: Z, y: J });
            return cs(be, Ce);
          }
          return cs(be);
        },
        ...
        (T) => {
          ...
          if (!E) {
            ...
            if (Y.closest('.layout-page-header') || Y.closest('.layout-page-footer')) {
              (T.preventDefault(),
                C.current && (C.current.setSelection(0), C.current.focus(), U(true)));
              return;
            }
          }
          let q = T.target;
          if (q.classList.contains('layout-table-resize-handle')) {
            ...
            if (Z) {
              let J = Z.state.doc.resolve(Se.current + 1);
              for (let ce = J.depth; ce >= 0; ce--) {
                let we = J.node(ce);
                if (we.type.name === 'table') {
                  let be = we.attrs.columnWidths;
                  be &&
                    be[Y] !== void 0 &&
                    be[Y + 1] !== void 0 &&
                    (pe.current = { left: be[Y], right: be[Y + 1] });
                  break;
                }
              }
            }
          ...
          if (
            ...
            let Z = C.current.getView();
            if (Z) {
              let J = Z.state.doc.resolve(We.current + 1);
              for (let ce = J.depth; ce >= 0; ce--) {
                let we = J.node(ce);
                if (we.type.name === 'table') {
                  let be = null,
          ...
          if (q.classList.contains('layout-table-edge-handle-right')) {
            ...
            let Z = C.current.getView();
            if (Z) {
              let J = Z.state.doc.resolve(gn.current + 1);
              for (let ce = J.depth; ce >= 0; ce--) {
                let we = J.node(ce);
                if (we.type.name === 'table') {
                  let be = we.attrs.columnWidths;
          ...
          if (X) {
            ...
            if (Y !== void 0) {
              let Z = parseInt(Y, 10);
              (C.current.setNodeSelection(Z), v(D(X, Z)), N([]), G(null));
            }
            (C.current.focus(), U(true));
            return;
          }
          ...
          if (te !== null) {
            ...
            if (Y) {
              let Z = Math.max(0, Y.state.doc.content.size - 1);
              (C.current.setSelection(Z), (de.current = Z), (ie.current = true));
            }
          }
          (C.current.focus(), U(true));
        },
      ...
      qe = useCallback(
        (T) => {
          if (ue.current) {
            T.preventDefault();
            let X = T.clientX - Ee.current;
            if ($e.current) {
              let te = parseFloat($e.current.style.left);
              (($e.current.style.left = `${te + X}px`), (Ee.current = T.clientX));
              let Y = Math.round(X * 15),
                Z = 300,
                J = pe.current.left + Y,
                ce = pe.current.right - Y;
              J >= Z && ce >= Z && (pe.current = { left: J, right: ce });
            }
            return;
          }
          if (Qe.current) {
          ...
          if (!ie.current || de.current === null || !C.current || !K.current) return;
          ...
          if (le.current !== null) {
            ...
            if (hn.current !== null && ne === hn.current) {
              ...
              else if (Math.abs(T.clientX - Xt.current) >= Qo) {
                ((Ft.current = true),
      ...
      bo = useCallback(() => {
        if (ue.current) {
          ...
          if (T) {
            let ne = Se.current,
              q = me.current,
              { left: X, right: te } = pe.current,
              Y = T.state.doc.resolve(ne + 1);
            for (let Z = Y.depth; Z >= 0; Z--) {
              let J = Y.node(Z);
              if (J.type.name === 'table') {
                let ce = Y.before(Z),
                  we = T.state.tr,
                  be = [...J.attrs.columnWidths];
                ((be[q] = X),
                  (be[q + 1] = te),
                  we.setNodeMarkup(ce, void 0, { ...J.attrs, columnWidths: be }));
                let Ce = ce + 1;
                (J.forEach((Fe) => {
                  let Le = Ce + 1,
                    Ye = 0;
                  (Fe.forEach((Xe) => {
                    ...
                    if (Ye === q || Ye === q + 1) {
                      let dt = Ye === q ? X : te;
                      we.setNodeMarkup(we.mapping.map(Le), void 0, {
                        ...Xe.attrs,
                        width: dt,
                        widthType: 'dxa',
                        colwidth: null,
          ...
          if (T) {
            ...
            for (let Y = te.depth; Y >= 0; Y--) {
              ...
              if (Z.type.name === 'table') {
                let J = te.before(Y),
                  ce = T.state.tr,
                  we = [...Z.attrs.columnWidths];
                ((we[q] = X), ce.setNodeMarkup(J, void 0, { ...Z.attrs, columnWidths: we }));
                let be = J + 1;
                (Z.forEach((Ce) => {
                  let Fe = be + 1,
                    Le = 0;
    ...
    useEffect(
      () => (
        ...
        window.addEventListener('mouseup', bo),
        () => {
          (window.removeEventListener('mousemove', qe), window.removeEventListener('mouseup', bo));
        }
      ),
      [qe, bo]
    );
    let Zi = useCallback(
        (T) => {
          ...
          if (ne) {
            ...
            if (q && C.current) {
              ...
              if (X) {
                let te = null;
                (X.state.doc.descendants((Y, Z) => {
                  if (te !== null) return false;
                  if (Y.type.name === 'paragraph' && Y.attrs.bookmarks?.some((ce) => ce.name === q))
                    return ((te = Z), false);
                }),
                  te !== null && (Qn(te), C.current.setSelection(te + 1)));
              }
            }
            return;
          }
            ...
            if (q !== null) {
              ...
              if (te) {
                let { doc: Y } = te.state,
                  ...
                  J = Z.parent;
                if (J.isTextblock) {
                  let ce = J.textContent,
                    we = Z.parentOffset,
                    be = we;
                  for (; be > 0 && /\w/.test(ce[be - 1]); ) be--;
                  let Ce = we;
                  for (; Ce < ce.length && /\w/.test(ce[Ce]); ) Ce++;
                  let Fe = Z.start() + be,
                    Le = Z.start() + Ce;
      ...
      gt = useCallback(
        (T) => {
          u || T.target.closest('.docx-comments-sidebar') || (C.current?.focus(), U(true));
        },
        [u]
      ),
      ht = useCallback((T) => {
        let ne = T.relatedTarget;
        (ne && O.current?.contains(ne)) || U(false);
      }, []),
      tr = useCallback((T, ne, q) => {
        let X = C.current?.getView();
        if (X)
          try {
              ...
              if (
                ...
                for (let xt of Z) {
                  ...
                  if (q >= dt.top && q <= dt.bottom) {
                    J = xt.querySelector('.layout-page-content');
                    break;
                  }
                }
                if ((J || (J = Z[Z.length - 1].querySelector('.layout-page-content')), !J)) return;
                let ce = J.getBoundingClientRect(),
                  we = (ne - ce.left) / c,
                  be = (q - ce.top) / c,
                  Ce = 914400 / 96,
                ...
                let Z = At(ne, q);
                if (Z === null || Z === T || Z === T + 1) return;
                let J = X.state.tr;
                if (Z <= T)
                  ((J = J.delete(T, T + te.nodeSize)),
                    (J = J.insert(Z, te)),
                    C.current?.setNodeSelection(Z));
                else {
                  J = J.delete(T, T + te.nodeSize);
                  let ce = Z - te.nodeSize;
                  ((J = J.insert(Math.min(ce, J.doc.content.size), te)),
                    C.current?.setNodeSelection(Math.min(ce, J.doc.content.size - 1)));
                }
                X.dispatch(J);
              }
            } catch {}
      ...
      Qi = useCallback(
        (T) => {
          if (!u) {
            if (
              (C.current?.isFocused() || (C.current?.focus(), U(true)),
    ...
    useEffect(() => {
      let T = () => {
        let ne = C.current?.getView();
        ne && (c$1(), ls(), Pt(ne.state), Bt(ne.state));
      };
      return (
    ...
    return jsxs('div', {
      ref: (T) => {
        ((O.current = T),
          typeof W == 'function' ? W(T) : W && typeof W == 'object' && (W.current = T));
      },
      ...
      children: [
        ...
        jsxs('div', {
          ...
          children: [
            ...
            w && jsx('div', { className: 'paged-editor__plugin-overlays', style: ab, children: w }),
          ],
        }),
        L,
      ],
  ...
  vu = memo(fb);
var bb = lazy(() => import('./FindReplaceDialog-AWQXKEUQ.js')),
  yb = lazy(() => import('./HyperlinkDialog-BA25XUT5.js')),
  xb = lazy(() =>
...
function Cb({ mode: e, onModeChange: t }) {
  let [n, o] = useState(false),
    [r, i] = useState(false),
    a = useRef(null),
    s = useRef(null),
    [l, u] = useState({ top: 0, left: 0 }),
    d$1 = Ru.find((c) => c.value === e);
  return (
    useEffect(() => {
...
var Pu = forwardRef(function (
  ...
  let [ue, Ee] = useState([]),
    ...
    Ft = useCallback(() => {
      ...
      let re = [];
      for (let ge of se) {
        let Pe = re[re.length - 1];
        Pe && Pe.revisionId === ge.revisionId && Pe.type === ge.type && Pe.to === ge.from
          ? ((Pe.text += ge.text), (Pe.to = ge.to))
          : re.push({ ...ge });
      }
      Ge(re);
    }, []);
  ...
  let le = yd(n || null, { maxEntries: 100, groupingInterval: 500, enableKeyboardShortcuts: true }),
    hn = useRef(false);
  useEffect(() => {
  ...
  let Xt = useMemo(() => {
    ...
    Qo = useMemo(() => wd(false, r), []),
    $t = useMemo(() => [Qo, ...(Ie ?? [])], [Qo, Ie]),
    Oe = useRef(null),
    ot = useRef(null),
    Lt = useRef(null),
    Wt = useRef(null),
  ...
  let He = useCallback(
      () => (v$1 && ot.current ? ot.current.getView() : Oe.current?.getView()),
      [v$1]
    ),
    qe = useCallback(() => {
      v$1 && ot.current ? ot.current.focus() : Oe.current?.focus();
    }, [v$1]),
    bo = useCallback(() => {
      v$1 && ot.current ? ot.current.undo() : Oe.current?.undo();
    }, [v$1]),
    Zi = useCallback(() => {
      v$1 && ot.current ? ot.current.redo() : Oe.current?.redo();
    }, [v$1]),
    gt = m(),
    ht = m$1();
  (useEffect(() => {
    ...
    (M((h) => ({ ...h, isLoading: true, parseError: null })),
      (async () => {
        try {
          let h = await q(t);
          (le.reset(h),
            M((H) => ({ ...H, isLoading: !1, parseError: null })),
            o(h).catch((H) => {
              console.warn('Failed to load document fonts:', H);
            }));
        } catch (h) {
          let H = h instanceof Error ? h.message : 'Failed to parse document';
          (M((ae) => ({ ...ae, isLoading: false, parseError: H })),
  ...
  (useEffect(() => {
    if (!y$1.isLoading && le.state) {
      let x = setTimeout(() => {
        (Ft(), tr.current || ((tr.current = true), Ge((h) => (h.length > 0 && Se(true), h))));
      }, 200);
    ...
    useEffect(() => {
      let x = Oe.current?.getView();
      x && Cd(De === 'suggesting', x.state, x.dispatch, r);
    }, [De, r]));
  let Nt = useCallback((x) => (le.push(x), x), [le]),
    Jt = useCallback(
      (x) => {
        if ((Nt(x), i?.(x), de.current)) {
    ...
    yo = useCallback(
      (x) => {
        ...
        if (!x) {
          (ln(null),
            M((at) => ({ ...at, selectionFormatting: {}, pmTableContext: H, pmImageContext: ae })));
          return;
        }
        let { textFormatting: oe, paragraphFormatting: se } = x,
          ...
          Ke = Pe
            ? {
                type: Pe.numId === 1 ? 'bullet' : 'numbered',
                level: Pe.ilvl ?? 0,
                isInList: true,
                numId: Pe.numId,
              }
        ...
        if (
          ...
          h && x.hasSelection && !tt && !R)
        ) {
          let at = Jn.current,
            st = Zn.current;
          if (at && st) {
            let { from: Co } = h.state.selection,
              ia = at.querySelector('.paged-editor__pages');
            if (ia) {
              let Ps = ia.querySelector('.layout-page'),
                Qu = ia.querySelectorAll('span[data-pm-start]');
              for (let ep of Qu) {
                let aa = ep,
                  tp = Number(aa.dataset.pmStart),
                  np = Number(aa.dataset.pmEnd);
                if (Co >= tp && Co <= np) {
                  let op = aa.getBoundingClientRect(),
                    sa = st.getBoundingClientRect(),
                    rp = op.top - sa.top + at.scrollTop,
                    ip = Ps ? Ps.getBoundingClientRect().right - sa.left : sa.width / 2 + 408;
                  ln({ top: rp, left: ip });
                  break;
                }
              }
    ...
    Dn = ml({ document: le.state, onChange: Jt, onSelectionChange: (x) => {} });
  useEffect(() => {
    let x = (h) => {
          ...
          if ('$anchorCell' in se && typeof se.forEachCell == 'function') {
            ...
            if (ge.isInTable && ge.table) {
              ...
              ge.table.descendants((bt) => {
                (bt.type.name === 'tableCell' || bt.type.name === 'tableHeader') && (Pe += 1);
              });
              let Ke = 0;
              if (
                (se.forEachCell(() => {
                ...
                (h.preventDefault(), No(oe.state, oe.dispatch));
                return;
              }
            }
          }
  ...
  let Ji = useCallback(
      (x, h) => {
        let H = He();
        H && (Yr(x, h)(H.state, H.dispatch), qe());
      },
      [He, qe]
    ),
    Qi = useCallback(() => {
      let x = He();
      x && (wi(x.state, x.dispatch), qe());
    }, [He, qe]),
    ea = useCallback(() => {
      let x = He();
      x && (Kr(x.state, x.dispatch), qe());
    }, [He, qe]),
    ta = useCallback(() => {
      ie((x) => {
        if (!x) {
        ...
        if (h) {
          switch (x) {
            ...
            case 'splitCell':
              ...
              break;
            case 'borderAll':
              si(h.state, h.dispatch, wt.current);
              break;
            case 'borderOutside':
              li(h.state, h.dispatch, wt.current);
              break;
            case 'borderInside':
              ci(h.state, h.dispatch, wt.current);
              break;
            case 'borderNone':
            ...
            default:
              if (typeof x == 'object') {
                if (x.type === 'cellFillColor') xi(x.color)(h.state, h.dispatch);
                else if (x.type === 'borderColor') {
                  let H = x.color.replace(/^#/, '');
                  ((wt.current = { ...wt.current, color: { rgb: H } }),
                    Si(x.color)(h.state, h.dispatch));
                } else if (x.type === 'borderWidth')
                  ((wt.current = { ...wt.current, size: x.size }), ki(x.size)(h.state, h.dispatch));
                ...
                else if (x.type === 'cellVerticalAlign') di(x.align)(h.state, h.dispatch);
                else if (x.type === 'cellMargins') ui(x.margins)(h.state, h.dispatch);
                else if (x.type === 'cellTextDirection') pi(x.direction)(h.state, h.dispatch);
                else if (x.type === 'toggleNoWrap') fi()(h.state, h.dispatch);
                else if (x.type === 'rowHeight') mi(x.height, x.rule)(h.state, h.dispatch);
                else if (x.type === 'toggleHeaderRow') gi()(h.state, h.dispatch);
                else if (x.type === 'distributeColumns') hi()(h.state, h.dispatch);
                else if (x.type === 'autoFitContents') bi()(h.state, h.dispatch);
                else if (x.type === 'openTableProperties') A(true);
                else if (x.type === 'tableProperties') zo(x.props)(h.state, h.dispatch);
                        ...
                        for (let ge of ['top', 'bottom', 'left', 'right', 'insideH', 'insideV']) {
                          ...
                          Pe &&
                            (H.tableBorders[ge] = {
                              style: Pe.style,
                              size: Pe.size,
                              color: Pe.color?.rgb ? { rgb: Pe.color.rgb } : void 0,
                            });
                        }
                      }
                      if (se.tblStylePr) {
    ...
    ce = useCallback(
      (x) => {
        ...
        if (x === 'indent') {
          jr(h.state, h.dispatch) || zr()(h.state, h.dispatch);
          return;
        }
        if (x === 'outdent') {
          Ur(h.state, h.dispatch) || Or()(h.state, h.dispatch);
          return;
        }
        if (x === 'clearFormatting') {
          Eo(h.state, h.dispatch);
          return;
        }
        if (x === 'insertLink') {
          let re = ro(h.state),
        ...
        if (typeof x == 'object')
          switch (x.type) {
            case 'alignment':
              Hr(x.value)(h.state, h.dispatch);
              break;
            case 'textColor': {
              let re = x.value;
              typeof re == 'string'
                ? Bo({ rgb: re.replace('#', '') })(h.state, h.dispatch)
                : re.auto
                  ? Ir(h.state, h.dispatch)
                  : Bo(re)(h.state, h.dispatch);
              break;
            }
            case 'highlightColor': {
              let re = x.value ? u(x.value) : '';
              Fr(re || x.value)(h.state, h.dispatch);
              break;
            }
            case 'fontSize':
              Lr(f(x.value))(h.state, h.dispatch);
              break;
            case 'fontFamily':
              Br(x.value)(h.state, h.dispatch);
              break;
            case 'lineSpacing':
            ...
            case 'applyStyle': {
              ...
              if (ge) {
                let Pe = ge.resolveParagraphStyle(x.value);
                Ho(x.value, {
                  paragraphFormatting: Pe.paragraphFormatting,
                  runFormatting: Pe.runFormatting,
                })(h.state, h.dispatch);
              } else Ho(x.value)(h.state, h.dispatch);
              break;
            }
          }
      },
    ...
    be = useCallback(
      (x) => {
        ...
        (oe && x.displayText
          ? Ao(x.displayText, H, ae)(h.state, h.dispatch)
          : oe
            ? x.displayText && Ao(x.displayText, H, ae)(h.state, h.dispatch)
            : Ar(H, ae)(h.state, h.dispatch),
          ht.close(),
          qe());
      },
      [ht, He, qe]
    ...
    Ce = useCallback(() => {
      let x = He();
      x && (Dr(x.state, x.dispatch), ht.close(), qe());
    }, [ht, He, qe]),
    Fe = useCallback(
      (x) => (h) => {
        if (!le.state || R) return;
        let H = {
          ...le.state,
          package: {
            ...le.state.package,
    ...
    zt = useCallback(
      (x) => {
        let h = He();
        h && (x < 0 ? Do(-x, true)(h.state, h.dispatch) : Do(x, false)(h.state, h.dispatch));
      },
      [He]
    ),
    Hn = useCallback(
      (x) => {
        let h = He();
        h && Gr(x)(h.state, h.dispatch);
      },
      [He]
    ),
    xo = useCallback((x) => {}, []),
    So = useCallback(async () => {
      ...
      try {
        ...
        (h?.package?.document && (x.package.document.content = h.package.document.content),
          (x.package.document.comments = pe));
...
@page { margin: 0; size: auto; }
  ...
  useImperativeHandle(
    ...
    [le.state, y$1.zoom, y$1.currentPage, y$1.totalPages, So, Qt]
  );
  let { headerContent: ko, footerContent: wo } = useMemo(() => {
    ...
    Ku = useCallback(
      (x) => {
        ...
        let oe = `rId_new_${x}`,
          ...
          ge = new Map(H[re] ?? []);
        ge.set(oe, se);
        let Pe = x === 'header' ? 'headerReferences' : 'footerReferences',
          Ke = ae[Pe] ?? [],
          ...
          at = {
            ...
            package: {
              ...
              [re]: ge,
              document: H.document
                ? { ...H.document, finalSectionProperties: { ...ae, [Pe]: [...Ke, bt] } }
                : H.document,
            },
          };
        (Nt(at), z$1(x));
    ...
    ra = useCallback(
      (x) => {
        if (!v$1 || !le.state?.package) {
          z$1(null);
          return;
        }
        let h = le.state.package,
          H = h.document?.finalSectionProperties,
          oe = (v$1 === 'header' ? H?.headerReferences : H?.footerReferences)?.find(
            (ge) => ge.type === 'default'
          ),
          se = v$1 === 'header' ? 'headers' : 'footers',
          re = h[se];
        if (oe?.rId && re) {
          let ge = re.get(oe.rId),
            Pe = { type: v$1, hdrFtrType: 'default', ...ge, content: x },
            Ke = new Map(re);
          Ke.set(oe.rId, Pe);
    ...
    qu = useCallback(() => {
      ...
      let x = le.state.package,
        h = x.document?.finalSectionProperties,
        H = v$1 === 'header' ? 'headerReferences' : 'footerReferences',
        ae = v$1 === 'header' ? 'headers' : 'footers',
        oe = h?.[H],
    ...
    Zu = { display: 'flex', flex: 1, minHeight: 0, minWidth: 0, flexDirection: 'row' },
    Ju = { flex: 1, minHeight: 0, minWidth: 0, overflow: 'auto', position: 'relative' };
  return y$1.isLoading
    ...
    : y$1.parseError
      ? jsx('div', {
          ...
          style: or,
          'data-testid': 'docx-editor',
          children: jsx(hd, { message: y$1.parseError }),
        })
      : le.state
        ? jsx(pl, {
            children: jsx(cr, {
                                  ...
                                  jsx(vu, {
                                    ...
                                    sidebarOverlay: me
                                      ? jsx(kl, {
                                          ...
                                          onAcceptChange: (x, h) => {
                                            let H = Oe.current?.getView();
                                            H && (Wa(x, h)(H.state, H.dispatch), Ft());
                                          },
                                          onRejectChange: (x, h) => {
                                            let H = Oe.current?.getView();
                                            H && (_a(x, h)(H.state, H.dispatch), Ft());
                                          },
                                          isAddingComment: tt,
                    ...
                    children: [
                      ...
                      ht.state.isOpen &&
                        jsx(yb, {
                          ...
                          onClose: ht.close,
                          onSubmit: be,
                          onRemove: ht.state.isEditing ? Ce : void 0,
                      ...
                      F &&
                        jsx(xb, {
                          ...
                          onApply: (x) => {
                            let h = He();
                            h && zo(x)(h.state, h.dispatch);
                          },
                          currentProps: y$1.pmTableContext?.table?.attrs,
                        }),
                      U && jsx(Sb, { isOpen: U, onClose: () => Q(false), onApply: X }),
                      N &&
                        jsx(kb, {
                          isOpen: N,
                          onClose: () => j(false),
...
function C0(e = {}) {
  ...
  return (
    useEffect(() => {
      ...
      if (R)
        return (
          R.addEventListener('wheel', k, { passive: false }),
          () => {
            R.removeEventListener('wheel', k);
          }
        );
    }, [i, a, k]),
    useEffect(() => {
  ...
  let {
        ...
        try {
          if (navigator.clipboard && navigator.clipboard.read) {
            let S = await navigator.clipboard.read(),
              ...
              k = '';
            for (let R of S)
              (R.types.includes('text/html') && (w = await (await R.getType('text/html')).text()),
                R.types.includes('text/plain') &&
                  (k = await (await R.getType('text/plain')).text()));
            g && (w = '');
            let E = ka(w, k, r);
    ...
    m = useCallback(
      (g) => {
        if (!i) return;
        g.preventDefault();
        let S = ja(g, { cleanWordFormatting: r });
        if (S) {
          l.current = S;
...
`,
  jb = forwardRef(function ({ plugins: t, children: n, className: o = '' }, r) {
    ...
    let [l, u] = useState(null),
      ...
      [p, f] = useState(() => {
        let C = new Set();
        for (let B of t) ({ ...Xi, ...B.panelConfig }).defaultCollapsed && C.add(B.id);
        return C;
      }),
      [m] = useState(() => {
        let C = new Map();
        for (let B of t) {
          let ee = { ...Xi, ...B.panelConfig };
          C.set(B.id, ee.defaultSize);
        }
        return C;
      });
    (useEffect(() => {
      ...
      useEffect(() => {
        let C = t.filter((B) => B.styles).map((B) => Vu(B.id, B.styles));
      ...
      useEffect(() => Vu('plugin-host-base', ju), []));
    let b = useCallback(
        (C) => {
          if (!i) return;
          if (i.coordsAtPos(C)) {
            ...
            let { state: ee } = i,
              ...
              fe = ee.tr.setSelection(TextSelection.near(he));
      ...
      g = useCallback(
        (C, B) => {
          ...
          let { state: ee } = i,
            ...
            fe = Math.max(0, Math.min(C, he)),
            Ie = Math.max(0, Math.min(B, he)),
            Te = ee.tr.setSelection(TextSelection.create(ee.doc, fe, Ie));
          (i.dispatch(Te), i.focus());
        },
        [i]
      ),
    ...
    let E = useMemo(() => {
        let C = [];
        for (let B of t) B.proseMirrorPlugins && C.push(...B.proseMirrorPlugins);
        return C;
      }, [t]),
      R = useCallback((C) => {
        f((B) => {
    ...
    useEffect(() => {
      ...
      let C = () => {
        let he = l.pagesContainer,
          fe = he.querySelector('.layout-page');
        if (!fe) {
          V(null);
          return;
        }
        let Ie = l.getContainerOffset(),
          Te = fe.getBoundingClientRect(),
          Re = he.getBoundingClientRect(),
          P = (Te.right - Re.left) / l.zoom,
          I = Ie.x + P + 5;
        V(I);
      };
      C();
      ...
      return (
        ...
        () => {
          (window.removeEventListener('resize', B), ee.disconnect());
        }
      );
    }, [l]);
    let $ = useMemo(() => {
              ...
              C.push(
                jsx(
                  'div',
                  {
                    ...
                    'data-plugin-id': B.id,
                    children: B.renderOverlay(l, ee, i),
                  },
                  `overlay-${B.id}`
                )
              );
        ...
        for (let B of t) {
          ...
          let he = { ...Xi, ...B.panelConfig },
            fe = p.has(B.id),
            Ie = m.get(B.id) ?? he.defaultSize,
            Te = B.Panel,
            Re = c.states.get(B.id),
            P = _ !== null ? `${_}px` : 'calc(50% + 428px)';
          C.push(
            jsxs(
              'div',
              {
                className: `plugin-panel-in-viewport ${fe ? 'collapsed' : ''}`,
                style: { width: fe ? '32px' : `${Ie}px`, left: P },
                'data-plugin-id': B.id,
                children: [
                  he.collapsible &&
                    jsx('button', {
                      ...
                      onClick: () => R(B.id),
                      title: fe ? `Show ${B.name}` : `Hide ${B.name}`,
                      'aria-label': fe ? `Show ${B.name}` : `Hide ${B.name}`,
                      children: jsx('span', {
      ...
      O = useMemo(() => {
        ...
        for (let he of t) {
          ...
          let fe = he.panelConfig?.position ?? 'right';
          fe === 'left' ? C.push(he) : fe === 'bottom' ? ee.push(he) : B.push(he);
        }
        return { left: C, right: B, bottom: ee };
      }, [t]),
      K = (C) => {
        if (!C.Panel) return null;
        let B = { ...Xi, ...C.panelConfig },
          ee = p.has(C.id),
          he = m.get(C.id) ?? B.defaultSize,
          fe = C.Panel,
          Ie = c.states.get(C.id);
        return jsxs(
          'div',
          {
            className: `plugin-panel plugin-panel-${B.position} ${ee ? 'collapsed' : ''}`,
            style: {
              [B.position === 'bottom' ? 'height' : 'width']: ee ? '32px' : `${he}px`,
              minWidth: B.position !== 'bottom' ? (ee ? '32px' : `${B.minSize}px`) : void 0,
              maxWidth: B.position !== 'bottom' ? `${B.maxSize}px` : void 0,
              minHeight: B.position === 'bottom' ? (ee ? '32px' : `${B.minSize}px`) : void 0,
    ...
    return jsxs('div', {
      ...
      children: [
        O.left.length > 0 &&
          jsx('div', { className: 'plugin-panels-left', children: O.left.map(K) }),
        jsxs('div', {
          className: 'plugin-host-editor',
          children: [
            W,
            O.bottom.length > 0 &&
              jsx('div', { className: 'plugin-panels-bottom', children: O.bottom.map(K) }),
          ],
        }),
      ],
    });
...
export {
  ...
  Bu as y,
  Db as z,
}; //# sourceMappingURL=chunk-YSVFLLHN.js.map
//# sourceMappingURL=chunk-YSVFLLHN.js.map
