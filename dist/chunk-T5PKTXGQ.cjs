...
async function vt(e, t, n) {
  ...
  for (let p of e) {
    ...
    let c = `rId${l}`;
    (s.push(
      `<Relationship Id="${c}" Type="${chunkGILLFIXY_cjs.b.hyperlink}" Target="${a(p.href)}" TargetMode="External"/>`
    ),
      (p.rId = c));
  }
  s.length > 0 &&
...
function At(e, t, n) {
  ...
  if (e.package.headers)
    for (let [o, l] of e.package.headers.entries()) {
      let s = r.get(o);
      if (s && s.type === chunkGILLFIXY_cjs.b.header && s.target) {
  ...
  if (e.package.footers)
    for (let [o, l] of e.package.footers.entries()) {
      let s = r.get(o);
      if (s && s.type === chunkGILLFIXY_cjs.b.footer && s.target) {
        let p = s.target.startsWith('/') ? s.target.slice(1) : `word/${s.target}`,
          c = B(l);
        t.file(p, c, { compression: 'DEFLATE', compressionOptions: i });
      }
